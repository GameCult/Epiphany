...
- Idunn, not Codex/Self/Gjallar, owns daemon survival and service lifecycle physiology. `tools/epiphany_local_run.ps1 -Mode swarm-online-runbook` now generates an Idunn-owned service lifecycle runbook for the Epiphany hosted body: the artifact launches each sealed elevated child runbook with `Start-Process ... -PassThru`, records child exit codes, continues after child failure, runs aftercare audits, and exits nonzero if any child or aftercare step fails. Its wrapper JSON and one-line summary print the exact swarm-level `elevatedCommand=Start-Process PowerShell -Verb RunAs -Wait ...`, artifact SHA-256, aftercare commands, owner, hosted body, and private-state seal; proof `local-20260619-010151-373-2ba6717a` generated that operator command without mutating services. It still requires explicit elevated operator authority before service mutation.
