...
- observation distillation now keeps noisy tool/command/shell/model output bounded by selecting salient output lines and defaulting evidence kind to `tool-output` or `model-output` when the caller did not provide one
...
- Phase 5 semantic distillation/promotion is active:
  ...
  - the next bounded follow-up is promotion policy over richer deltas, not proving the retriever exists again
