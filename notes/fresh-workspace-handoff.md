...
- The next memory foundation must not build separate engines for repo graph and agent mind. Heartbeat cognition, role self-memory, repo architecture/dataflow, evidence scars, incubation, agency pressure, and context packets should converge through `EpiphanyMemoryGraph` profiles so one typed graph owns anchors, lifecycle, summaries, freshness, context cuts, and Qdrant embedding manifests. The repo profile producer maps accepted architecture/dataflow graph state into memory graph documents without scanner/Qdrant/model work and keeps bad topology visible as validation failure. The agent profile producer maps reviewed role dossier memories/goals/values into role_self/identity graph nodes without treating heartbeat rumination as durable memory. The next clean implementation slice is heartbeat cognition pressure import as provisional short-term/incubation/agency/candidate profile documents, still before sleep runner, Qdrant, CultNet, or prompt integration.
