...
- The next memory foundation must not build separate engines for repo graph and agent mind. Heartbeat cognition, role self-memory, repo architecture/dataflow, evidence scars, incubation, agency pressure, and context packets should converge through `EpiphanyMemoryGraph` profiles so one typed graph owns anchors, lifecycle, summaries, freshness, context cuts, and Qdrant embedding manifests. The repo profile producer now maps existing accepted architecture/dataflow graph state into memory graph documents without scanner/Qdrant/model work and deliberately keeps bad topology visible as validation failure. The next clean implementation slice is an agent profile producer over reviewed role self-memory and heartbeat cognition pressure, still before sleep runner, Qdrant, CultNet, or prompt integration.
