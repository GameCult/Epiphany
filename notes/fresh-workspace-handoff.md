...
fixed-lane coordinator decision policy/tests, and role-board projection
policy/tests live in core, live acceptance writes typed receipts keyed by runtime result id,
...
CRRC/coordinator read wrappers are deleted. The next cut is role-result
interpretation or acceptance policy extraction into `epiphany-core`. No Agile
