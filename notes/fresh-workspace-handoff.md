...
- Heartbeat participant scene id, personality timing, and mood timing now live as typed participant fields instead of durable `participant.extra` JSON pockets. Scheduler pacing and status projection read the typed fields first and keep legacy extra fallbacks only as sealed compatibility scars.
