...
- `notes/epiphany-repo-swarm-mvp-contract.md` corrects the repo-swarm MVP authority boundary: autonomous branch-local work inside the owned repo Body is in scope and desired; Bifrost gates publication/merge/escalation, not every local edit. Nine native repo-swarm front doors are now live: `epiphany-repo init --workspace <repo>` writes the repo-swarm init receipt and branch workbench plan; `epiphany-swarm online --workspace <repo>` seeds repo-local `.epiphany/local-verse.ccmp`, standing-faculty SoA, topology/liveness/tool/overview proof, and `.epiphany/swarm-online/repo-swarm-online-receipt.json`; `epiphany-work accept --workspace <repo> --from persona-or-bifrost --item <id>` routes a Persona/Bifrost item through CultMesh collaboration feedback into Imagination consensus discovery and writes `.epiphany/work/work-accept-<item>.json` with no Hands, durable-state, or publication authority granted; `epiphany-work plan --workspace <repo> --item <id> ...` writes a typed Imagination/Self action-plan receipt with objective, command, changed paths, commit message, verification asks, stop conditions, and rollback hints; `epiphany-work run --workspace <repo> [--item <id>]` persists a repo-local runtime-spine Substrate Gate read/snapshot grant, Hands intent, and Hands review with `decision=queued-for-adoption`; `epiphany-work adopt --workspace <repo> --item <id> --from-plan <receipt>` promotes that queued review to an approved branch-local Hands gate using the typed plan evidence; `epiphany-work execute --workspace <repo> --item <id> --from-plan <receipt>` consumes the approved gate on an `epiphany/*` branch, runs the planned command in the repo Body, captures stdout/stderr artifacts, stages only planned changed paths, commits, and records Hands patch/command/commit receipts; `epiphany-work publish --workspace <repo> --item <id> ...` requires Hands commit proof plus Soul verification and Mind/review refs, routes Bifrost publication/GitHub publication receipts, records a Hands PR receipt, and keeps `upstreamMainSynced=false` / `mergeAuthorized=false`; `epiphany-work sync --workspace <repo> --item <id> --upstream-ref origin/main --merge-receipt <ref>` requires an explicit maintainer/Bifrost merge receipt, verifies the published commit is an ancestor of upstream main, and writes the sync receipt with `upstreamMainSynced=true` only after git proves it. Fresh-repo smokes proved init, online, accept, plan, run, adopt-from-plan, execute-from-plan, publish, and sync with `privateStateExposed=false`; the plan smoke proved `execute --from-plan` created a branch-local commit from the typed plan packet, and the sync smoke proved `upstream-main-not-synced` before fast-forwarding main and `upstream-main-synced` after `git merge --ff-only`. Pre-adoption gates still refuse record-pass. Next missing organ: scheduler physiology for advancing safe branch-local work without manual CLI stepping, then deeper Persona-to-plan automation so plan packets are model/Imagination-authored rather than operator-authored.
