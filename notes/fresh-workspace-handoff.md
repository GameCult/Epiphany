...
- `notes/epiphany-repo-swarm-mvp-contract.md` corrects the repo-swarm MVP authority boundary: autonomous branch-local work inside the owned repo Body is in scope and desired; Bifrost gates publication/merge/escalation, not every local edit. Future migration plans must make that autonomy typed, branch-contained, receipt-backed, interruptible, private-state-sealed, and cross-body-safe rather than turning Epiphany into a human-permission queue. The first native front door is now `cargo run --manifest-path .\epiphany-core\Cargo.toml --bin epiphany-repo -- init --workspace <repo>`; it delegates to the reviewed startup-only birth runner, writes `.epiphany/repo-init/repo-swarm-init-receipt.json`, prepares repo-local state paths, and emits a planned `epiphany/<swarm-id>/<topic>` branch receipt unless `--create-branch` or `--switch-branch` grants local branch mutation. The second native front door is `cargo run --manifest-path .\epiphany-core\Cargo.toml --bin epiphany-swarm -- online --workspace <repo>`; it requires the init receipt, seeds repo-local `.epiphany/local-verse.ccmp`, bootstraps a repo-local standing-faculty agent store when absent, refreshes SoA, reads CultMesh topology/liveness/tool/overview surfaces, and writes `.epiphany/swarm-online/repo-swarm-online-receipt.json`. Fresh-repo smoke proved 7 SoA rows, 7 cluster/private-Verse/daemon rows, 19 all-agent tools, and `privateStateExposed=false`. Next missing organ: `epiphany-work accept --workspace <repo>`.
