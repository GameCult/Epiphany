...
- 2026-06-20: Gjallar/swarm-overview now exposes repo-work overview history as first-class compact sight, and Persona's public Eve surface lowers the same queue for peer-readable collaboration. `SwarmOverviewOutput` and `SwarmTriageOutput` include `repoWorkOverviewCount`, `repoWorkOverviewRows`, `repoWorkOverviewTuiRows`, `latestRepoWorkOverview`, `latestRepoWorkGate`, `latestRepoWorkBlocker`, and `latestRepoWorkNextSafeMove`; the shared `swarmActionRows` queue gets priority 35 `repo-work-overview` rows owned by Gjallar, hosted body `repo-work`, authority gate `repo.work.overview`, effect class `repo-work-readback`, no mutation, no elevation, proof-bundle ref, and private-state seal. `eve-surfaces`, direct `connect-eve`, and the globally invokable Persona `eve-connect` daemon tool readback now expose public `REPO-WORK-PEER` rows with item, gate, blocker, next safe move, branch, publication, sync, and `private=false`. Disposable proof `.epiphany-smoke\gjallar-repo-work-queue-20260620-012027` proved typed event-history queue enumeration; disposable proof `.epiphany-smoke\eve-repo-work-queue-20260620-013322` proved two accepted items through Persona Eve surface, direct Eve connection, and Persona tool invocation readback with queue count 2 and no private-state exposure. The next cut is not another sight surface: wire this queue into a usable repo-swarm run surface and deepen Persona-to-Imagination plan derivation.
