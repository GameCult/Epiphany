...
option-ordinal, and worker-job identities. The separate binding type, schema
constant, runtime registration, third CAS envelope, replay collision branch,
and duplicate worker-result/launch hashes are gone. The referenced launch and
result remain immutable typed documents and promotion strongly reads them when
atomically inserting the proposal/request pair. Runtime advances to v30. The
existing full keyed lifecycle passes promotion, concurrent launch, retry
ancestry, Modeling admission, concurrent Persona/Hands, and Verification
(1/1); epoch refusal passes (1/1). Package cleanup removes 677.6 MiB. The cut
removes 53 net maintained lines. Atlas remains untouched and no live
inter-swarm collaboration has run.

Exact `65bf044f` removes fixed schema-version strings and unread contract prose
from Hands patch, command, and commit receipts. CultCache already owns their
type/schema identity; exact intent, review, Substrate Gate grant, runtime job,
paths or command, consequence summary, time, and immutable envelope remain the
admission proof. Fourteen definition-only shadow type constants and one
callerless reorient display enum are also gone. Runtime advances to v29 and old
writable state refuses. The existing keyed lifecycle proof passes Hands through
concurrent Persona admission into exact Verification work (1/1); epoch refusal
passes (1/1). Package cleanup removes the generated 678.6 MiB. The cut removes
67 net maintained lines. Model Atlas is untouched: component proofs are not
collaboration acceptance, and no live inter-swarm exchange has run.

Exact `695af6c6` removes thirteen Cargo dependency declarations with no
consumer in their owning package, the tracked `.gitignore` that kept the
extinct `epiphany-openai-auth-spine` directory alive, and a production-only
unused import. The release bundle no longer directly owns nine libraries used
only by leaf crates or not used anywhere in its executable sources; affected
leaf/dev manifests drop unused `serde_json`, `futures`, `pretty_assertions`,
and a dev-only state-model edge. The lock graph contracts from 757 to 732
packages and the cut removes 284 net manifest, lock, and source lines. All nine
production binaries compile individually; release construction and the OpenAI
Codex spine compile; core and OpenAI-runtime library test targets compile.
Package-scoped cleanup removes 56,581 disposable files and 124.2 GiB from the
five packages compiled during verification. The shared target is 108.42 GiB,
the prebuilt state tool remains healthy, and no alternate target root was
created. Model Atlas is untouched; live inter-swarm collaboration remains
unproven.
...
context IDs; its linked-event vector repeated that same effect ID and its
relationship field was always `None`. Tool-receipt v1 retains exact result JSON
or error; its sealed-raw-result reference was always `None` and had no producer
or reader. Mind/runtime writable epochs advance to v10/v26 with no migrator or
compatibility reader. Tool adapter compiles; tool MCP 8/8, Persona conversation
and retention 5/5, old-epoch refusal 1/1, OpenAI runtime 23/23, and the packaged
model-runtime entrypoint pass. The cut removes 10 net maintained lines. Atlas
is untouched and live inter-swarm collaboration remains unproven.
...
constants. Acceptance/runtime-link aggregates, duplicate graph and frontier
DTOs, global retrieval and scratch state, generic job bindings, and churn state
had no constructor, reader, writer, registry entry, test, or downstream
consumer. Their live concepts already belong to keyed Mind/runtime receipts,
keyed RepoModel documents, sealed reasoning passes, and exact runtime jobs. The
standalone worker wrapper and direct model-request writer likewise had no
caller; the release-owned observed worker path remains. State-model, core,
OpenAI-runtime, and the packaged model-runtime entrypoint compile through the
shared target. The cut removes 291 maintained lines, changes no writable
schema, and leaves Atlas untouched. Live inter-swarm collaboration remains
unproven.
...
objective drafts—had no constructor, writer, admission owner, or consumer, yet
were read and injected empty into every reasoning projection. Their document
types, registry/validation/projection branches, reasoning field, and 256-line
issue-tracker-shaped state-model vocabulary are gone. Real planning remains in
frontier planning requests, candidates, Mind review, adopted decisions,
checkpoints, Hands receipts, and their exact admission paths. Mind/runtime
writable epochs advance to v9/v24. State model and core compile; sealed
reasoning 8/8, old-epoch refusal, model runtime 23/23, and coordinator 5/5 pass.
The cut removes 350 net maintained lines without touching Atlas.
...
provenance. Step count, provider echo, store path, artifact lists, arbitrary
metadata, and final-job echo had no reader and are gone. Operator artifacts
remain in the non-authoritative summary. The published JSON schema now matches
the Rust receipt; its previous `additionalProperties: false` contract omitted
the live resident fields. Runtime writable state advances to v20. JSON schemas
...
Continue the registered-type, field-cargo, and test-liability audit after exact `2a435eb5`. Model Atlas remains protected
...
lines; `runtime_spine.rs` reached 9,819 lines with 54 public functions. The atomic
...
Exact `777dd1a5` deletes `epiphany-tool-mcp-runtime smoke`, a callerless command
that printed fixed JSON constants and observed no runtime state. Its sole test
compared those constants with themselves and checked that the prose did not say
...
runtime spine toward its current v7 shape. Launches no longer rewrite a shared singleton merely to
...
Exact `a319146b` deletes the public non-atomic worker-launch path that only test
fixtures called; executor fixtures now seed through the canonical preparation
path. Exact `76fd259b` deletes two source-layout tests while retaining actual
CLI refusal behavior. Exact `b95a3266` removes the superseded, unpackaged
`epiphany-openai-spine` debug binary and two tests that animated its private
status helper.

Exact `6f93134d` makes the release bundle the sole package owner of every
