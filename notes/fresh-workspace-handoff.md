...
- Repo personality initialization now has a terrain-reduced plan in `notes/repo-personality-projection-layer-plan.md`. The intended first implementation seam is a native `epiphany-repo-personality` binary that scouts local repos, scores body taxonomy/history temperament/memory doctrine, and emits reviewable typed terrain/profile/role-projection artifacts before any Ghostlight-shaped role memory is mutated.
