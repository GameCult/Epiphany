...
and role-result interpretation/self-persistence review policy/tests live in core, live acceptance writes typed receipts keyed by runtime result id,
...
CRRC/coordinator read wrappers are deleted. The next cut is acceptance/application
policy extraction into `epiphany-core`, followed by runtime/job lifecycle mirror
cleanup. No Agile confetti. No new balcony on the swaying tower.
