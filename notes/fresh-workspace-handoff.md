...
- 2026-06-20: Gjallar/swarm-overview now exposes the latest repo-work overview as first-class compact sight. `SwarmOverviewOutput` and `SwarmTriageOutput` include `repoWorkOverviewCount`, `repoWorkOverviewRows`, `repoWorkOverviewTuiRows`, `latestRepoWorkOverview`, `latestRepoWorkGate`, `latestRepoWorkBlocker`, and `latestRepoWorkNextSafeMove`. The shared `swarmActionRows` queue also gets priority 35 `repo-work-overview` rows owned by Gjallar, hosted body `repo-work`, authority gate `repo.work.overview`, effect class `repo-work-readback`, no mutation, no elevation, proof-bundle ref, and private-state seal. Disposable proof `.epiphany-smoke\gjallar-repo-work-overview-20260620-011119` created a fresh repo, ran `init -> online -> accept -> overview -> gjallar`, and read `latestRepoWorkOverview=repo-work-overview-gjallar-request`, gate `awaiting-plan`, blocker `plan-receipt-missing`, one compact `REPO-WORK` row, non-mutating/non-elevated action row, and `privateStateExposed=false`. This completes latest-key presentation, not multiple-work-item queue enumeration; build a typed history/queue loader next.
