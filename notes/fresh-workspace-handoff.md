...
and fixed-lane coordinator decision policy/tests live in core, live acceptance writes typed receipts keyed by runtime result id,
...
CRRC/coordinator read wrappers are deleted. The next cut is role-board
projection policy or role-result interpretation extraction into `epiphany-core`.
