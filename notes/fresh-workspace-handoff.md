...
- 2026-06-21: The local wrapper mouth now forwards Rust-owned repo-work map rows for `gjallar` and `swarm-triage`. Live wrapper proofs `local-20260621-004816-315-24fe14d4` and `local-20260621-004815-947-b647d7a9` printed `repoWorkMapRows`, `repoWorkSemanticRows`, family/path/branch/stage/gate lens rows, `repoWorkOverviewRows`, and `repoWorkPublicProofRows` in the one-line operator summary while `privateStateExposed=false`; the current live store had no accepted repo-work rows, so each rendered `none`. PowerShell only lowers existing Rust TUI arrays and does not reinterpret repo-work truth or gain scheduler/publication/Mind authority.
