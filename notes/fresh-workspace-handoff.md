...
- Durable Epiphany state types now live in root crate `epiphany-state-model`. `codex-protocol` re-exports them only so old rollout/API callers keep compiling; do not add new durable `Epiphany*` state structs under `vendor/codex`. `epiphany-core` imports the native model directly. The remaining `epiphany-core` dependency on `codex-protocol` is now limited to Codex rollout/event reconstruction and token-pressure input, and that is the next wound to cut.
...
Continue with the actual whale-carcass cut: remove the remaining
`epiphany-core` dependency on `codex-protocol` by extracting or relocating the
Codex rollout adapter and token-pressure input, then keep starving the old model
...
runtime boundary while Codex survives only as subscription auth/model routing.
