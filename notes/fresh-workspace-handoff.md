...
- Current Mom-ready blocker: the normal 180-second MVP pass reaches Face -> coordinator -> detached runtime -> heartbeat sleep, but the modeling worker produces no typed result, no tool intents, and empty stdout/stderr before the runtime timeout writes `Worker runtime timed out after 180 seconds.` Native status correctly projects that failure through role result, jobs, roles, and coordinator source signals. Timeout ownership now seals the inner `openai-worker-*` job as well as the outer role worker; use `epiphany-runtime-spine list-jobs` to verify lifecycle invariants. Direct tiny model-turn transport succeeds through the ChatGPT subscription path, and worker events now show `request_prepared` and `transport_ready` before silence. The current suspect is worker prompt/launch shape: about 9.2k instruction chars plus 6.7k input chars still reaches transport and then emits no SSE before timeout even with low reasoning. Next cut should reduce or reshape the worker request, or add deeper stream payload diagnostics if a smaller worker request still stalls.
