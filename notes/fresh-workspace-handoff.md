...
- 2026-06-18: `swarm-overview` action rows now forward sealed artifact witness data from the selected service-lifecycle attention receipt. The non-mutating priority-40 readback row carries the service-manager artifact ref/status/hash/private flag instead of saying `artifact=none`; live wrapper proof `local-20260618-213553-947-eb284491` printed `artifactStatus=external-ref`, `sha256=none`, and `artifact=service-manager://windows/epiphany-cluster-daemon-services/cluster-execution-readiness` for that row while keeping `mutates=False`, `elevated=False`, and the priority-50 elevated runbook row unchanged.
