...
acceptance bundle policy/tests live in core, live acceptance writes typed receipts keyed by runtime result id,
...
CRRC/coordinator read wrappers are deleted. The next cut is runtime/job lifecycle
mirror cleanup: reassess whether `jobBindings` can shrink further or be deleted
now that `runtime_links` and runtime-spine read-back own live result lookup. No
Agile confetti. No new balcony on the swaying tower.
