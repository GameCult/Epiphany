...
- Native `epiphany-runtime-spine` is the first Codex-independent runtime vertebra. It owns typed CultCache documents for runtime identity, sessions, jobs, job results, and events; opens/completes native jobs; projects job-result counts; and can emit a framed CultNet hello message advertising the native document contract. Treat Codex app-server as a temporary evacuation bridge for launch/read-back until runtime sessions, jobs, and specialist results migrate to this spine.
