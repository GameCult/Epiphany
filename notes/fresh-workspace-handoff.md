...
- `notes/epiphany-memory-graph-unified-plan.md` is the current memory architecture direction drawn from `Fractal Domains And The Cache That Bites` plus the VoidBot memory scar tissue. The repo semantic architecture/dataflow graph and agent memory are one `EpiphanyMemoryGraph` substrate with repo, role, short-term, incubation, agency, candidate-intervention, and evidence profiles. The first native skeleton is landed: `epiphany-state-model` owns shared graph document types, `epiphany-core::memory_graph` owns stable ids, validation, freshness, context cuts, and CultCache persistence, and `epiphany-memory-graph` can status/validate/context/semantic-context/smoke/index the store. `epiphany-core::semantic_cache` now owns the shared Qdrant/Ollama transport used by workspace retrieval and memory graph indexing, so Qdrant remains a rebuildable contribution cache for typed graph documents, not truth. Runtime role results expose typed `memoryPatchCandidates`; Modeling `roleAccept` applies them to `state/memory-graph.msgpack`; native role/reorientation launches now prefer that persisted memory graph for bounded typed `memoryContext` cuts and only fall back to legacy accepted thread graphs internally to derive that packet when the store is missing. Worker launch documents no longer carry full `graphs` cargo. The old `thread/epiphany/propose` read route no longer returns replacement `graphs`; it loads the persisted memory graph for loaded threads, exposes graph growth as typed `memoryPatchCandidates`, withholds frontier updates that depend on unapplied memory growth, and native `EpiphanyMapProposal` no longer carries replacement `graphs`. Public `ThreadEpiphanyUpdatePatch`, native `EpiphanyStateUpdate`, and native `EpiphanyPromotionInput` no longer carry `graphs`; role worker `statePatch` documents deny unknown fields and no longer carry `graphs`, so Modeling `statePatch.graphs` is rejected at parse time; generic update/promote bridge mutation mapping cannot construct graph replacement. `thread/epiphany/view`, `thread/epiphany/freshness`, `thread/epiphany/propose`, `thread/epiphany/context`, and `thread/epiphany/graphQuery` now prefer the persisted memory graph for loaded threads through read-route store adapters and project back into legacy graph-shaped response lenses only through `epiphany-codex-bridge::legacy_graph_compat`. `notes/repo-fractal-dataflow-cache-plan.md` and `notes/agent-memory-fractal-cache-plan.md` are profile plans, not rival organs.
...
- Epiphany memory graph loading and runtime-spine path derivation now belong to `epiphany-codex-bridge::state`. Vendored app-server read routes may decide whether a loaded thread exists, but they no longer own memory graph or runtime-spine store path/load mechanics.
- Epiphany job launch/interrupt policy and state-update application now belong to `epiphany-codex-bridge::mutation_service`: heartbeat launch planning, runtime-spine job opening, binding/runtime-link mutation, interrupt binding cleanup, and typed patch apply are out of vendored `CodexThread`. `CodexThread` now exposes only the narrow Codex-session host seam for reading current Epiphany state/reference turn and persisting an already-mutated snapshot into rollout/session storage.
- Epiphany retrieval status, indexing, and retrieval execution now belong to `epiphany-codex-bridge::retrieve`; vendored `CodexThread` only exposes cwd/codex-home host facts for that path.
- Vendored app-server no longer imports `epiphany_core` directly. Retrieval query normalization and raw route job-launch DTO construction are bridge-owned adapters; the remaining direct native render call is in Codex core's model-context prompt fragment host seam.
- Coordinator automation input assembly and pre-compaction intervention text rendering now belong to `epiphany-codex-bridge`; vendored app-server keeps only host actions: submit Codex compaction, launch through bridge, steer active turns, and emit JSON-RPC notifications.
- Epiphany prompt-state boundary tags now live in `epiphany-state-model` and are re-exported through `codex-protocol`; Codex compatibility may wrap the prompt fragment, but it no longer owns those marker constants.
- Epiphany rollout snapshot anchoring now lives in typed `EpiphanyStateItem` constructors (`anchored_to_turn`, `for_optional_turn`, `out_of_band`) in `epiphany-state-model`. Codex core still physically writes rollout/session items, but live host code no longer hand-assembles the turn/out-of-band snapshot shape.
- Heartbeat adaptive pacing now lives as typed CultCache state on `EpiphanyHeartbeatStateEntry` instead of being written into `state.extra["adaptivePacing"]` as JSON. Status projection still emits JSON and reads the old extra pocket as a legacy fallback.
- Heartbeat participant scene id, personality timing, mood timing, and pending-turn cooldown multipliers now live as typed fields instead of durable `participant.extra` / `pendingTurn.extra` JSON pockets. Scheduler pacing and status projection read the typed fields first and keep legacy extra fallbacks only as sealed compatibility scars.
- Heartbeat cognition appraisals and reactions now live as typed CultCache fields instead of `Option<Value>` blobs. The mood-timing scheduler consumes typed appraisals directly; legacy cognition migration parses old appraisal/reaction JSON only when loading prior stores.
- Heartbeat cognition candidate interventions now live as typed review-gated speech candidates instead of `Option<Value>` item lists. The memory graph heartbeat profile reads typed candidate items directly; legacy cognition migration parses old candidate JSON only when loading prior stores.
- Heartbeat cognition memory resonance now lives as typed resonance pairs instead of an `Option<Value>` blob. Incubation and memory graph import read typed pairs directly; JSON remains only for status/artifact projection.
- Heartbeat cognition sleep cycle now lives as typed physiology state instead of an `Option<Value>` blob. Legacy sleep JSON is parsed only during old-store migration; status and routine artifacts receive JSON projections.
- Heartbeat cognition bridge judgment now lives as typed durable state instead of an `Option<Value>` blob: syntheses, source coverage, topic saturation, refractory topics, tensions, and speech decision are typed fields. The memory graph heartbeat profile reads typed bridge decisions directly.
- Heartbeat cognition thought lanes now live as typed durable state instead of `Option<Value>` analytic/associative lane blobs.
- Heartbeat cognition incubation now lives as typed durable state instead of an `Option<Value>` theme blob. Source coverage, theme identity, evidence lineage, maturation, novelty, saturation, refractory cooling, priority, status, and explanatory strings are typed fields; memory graph import reads typed themes directly. Legacy cognition migration still parses old incubation JSON when loading prior stores; status and routine artifacts receive JSON projections of the typed document.
- Heartbeat durable document/schema types now live in `epiphany-core/src/heartbeat_state/heartbeat_documents.rs`, role/default construction lives in `epiphany-core/src/heartbeat_state/heartbeat_roles.rs`, persistence/validation lives in `epiphany-core/src/heartbeat_state/heartbeat_store.rs`, and status/artifact JSON projection lives in `epiphany-core/src/heartbeat_state/heartbeat_projection.rs`. The root `heartbeat_state.rs` keeps scheduling and Void cognition orchestration for the next cuts. Focused heartbeat tests and `epiphany-core` check pass after the splits.
...
- Thread projection now lives in `codex_message_processor/thread_projection.rs`: state-db/thread-store/rollout summary projection, `ConversationSummary` to API `Thread` mapping, thread-title attachment helpers, turn pagination, and rollout turn reconstruction. `codex_message_processor.rs` is about 954 lines after extraction and the Epiphany dispatch gate; it mostly owns JSON-RPC dispatch, processor construction, shared thread lookup/submit/error helpers, and lifecycle shutdown. Epiphany route enumeration lives in `codex_message_processor/epiphany_dispatch.rs`, not the root Codex dispatcher.
...
- The next memory foundation must not build separate engines for repo graph and agent mind. Heartbeat cognition, role self-memory, repo architecture/dataflow, evidence scars, incubation, agency pressure, and context packets should converge through `EpiphanyMemoryGraph` profiles so one typed graph owns anchors, lifecycle, summaries, freshness, context cuts, and Qdrant embedding manifests. Repo, agent, and heartbeat profile producers now map accepted graph truth, reviewed role dossier memories/goals/values, and provisional heartbeat short-term/incubation/agency/candidate pressure into memory graph documents without scanner/model work. Heartbeat import deliberately does not mark durable promotion. Compose now merges profile snapshots into one validated graph and rejects duplicate authority; `epiphany-memory-graph compose` writes a composed typed CultCache graph from typed source graph stores; `epiphany-memory-graph refresh` can invoke live agent-memory plus heartbeat-cognition producers from their typed stores into one graph store; `epiphany-core` has a native typed `EpiphanyThreadState` CultCache store seam; the bridge mirrors loaded Codex-thread Epiphany state into `state/thread-state.msgpack`; CultNet advertises logical `EpiphanyThreadState` plus `EpiphanyMemoryGraph` snapshot contracts; memory graph nodes/edges/summaries project into deterministic typed embedding documents; `epiphany-memory-graph manifest` writes the embedding manifest back to the typed graph store; `epiphany-memory-graph index` fully rebuilds a Qdrant cache collection through the shared semantic cache transport; `epiphany-memory-graph semantic-context` uses ordered Qdrant IDs to seed typed graph context while falling back to graph traversal when the cache is absent; native role/reorientation worker launch documents now prefer persisted typed `memoryContext` instead of full `graphs`; typed runtime role results can persist `memoryPatchCandidates` for graph/memory growth proposals; `epiphany-core` reviews/appends candidate domains/nodes/edges against the typed graph with lifecycle receipts; `epiphany-memory-graph review-candidate` / `apply-candidate` expose that law for typed candidate files; roleResult findings expose typed `memoryPatchCandidates` for pre-acceptance operator review; bridge Modeling `roleAccept` applies runtime-spine memory patch candidates into `state/memory-graph.msgpack` through the same graph law, bootstrapping from accepted thread graphs when needed; roleAccept responses expose typed `memoryPatchReviews`; role worker `statePatch` documents deny unknown fields and no longer carry `graphs`, so Modeling `statePatch.graphs` is rejected at parse time; `thread/epiphany/propose` loads the persisted memory graph for loaded threads and exposes typed `memoryPatchCandidates` instead of replacement `graphs`; native `EpiphanyMapProposal` no longer carries replacement `graphs`; and native `EpiphanyStateUpdate` / `EpiphanyPromotionInput` no longer have graph replacement slots. The next clean slice is removing remaining operator/UI assumptions that graph growth comes from thread-state graph replacement instead of the unified memory graph.
