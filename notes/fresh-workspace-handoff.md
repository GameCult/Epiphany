...
- Wrapper `swarm-overview` and `swarm-triage` one-line summaries now include service lifecycle receipt `artifactStatus`, operator aftercare, and compact `serviceExecutionFailedChecks`. Live artifacts `local-20260618-162841-275-6bc3ef3e` and `local-20260618-162629-280-1e132880` show both lifecycle attention rows, the operator-elevated runbook row with `aftercare=tools/epiphany_local_run.ps1 -Mode cluster-service-execution-audit`, and readiness/install/start/audit/stop failed checks, with private state sealed.
