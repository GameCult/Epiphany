...
- Heartbeat cognition memory resonance now lives as typed resonance pairs instead of an `Option<Value>` blob. Incubation and memory graph import read typed pairs directly; temporary JSON projection remains only for still-untyped thought-lane/bridge builders and status/artifact output.
...
- Heartbeat cognition bridge judgment now lives as typed durable state instead of an `Option<Value>` blob: syntheses, source coverage, topic saturation, refractory topics, tensions, and speech decision are typed fields. The memory graph heartbeat profile reads typed bridge decisions directly; only incubation and thought lanes remain as live cognition JSON pockets.
- Heartbeat cognition thought lanes now live as typed durable state instead of `Option<Value>` analytic/associative lane blobs. Temporary JSON projection remains only for still-untyped incubation-adjacent cognition helpers and status/artifact output; incubation is the last live cognition JSON pocket.
