...
- CultMesh is now the preferred Rust abstraction for that local store work. The Rust port lives in `E:\Projects\CultLib\crates\cultmesh-rs`, and Epiphany now depends on it directly from `epiphany-core`. The first integration is deliberately small: `EpiphanyCultMeshStatusEntry` round-trips through `.epiphany-smoke/cultmesh/epiphany-local.ccmp` using `cargo run --manifest-path .\epiphany-core\Cargo.toml --bin epiphany-cultmesh-smoke`. Future local dream/status/Verse work should start from CultMesh, not raw CultNet, unless the task is polishing the CultMesh internals themselves.
