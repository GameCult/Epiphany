...
- Manual rumination now has an explicit aftercare rule. A heartbeat/routine vigil is only physiology until a separate distillation pass reviews the receipts and decides whether map, handoff, evidence, or role self-memory should change. Do not mistake repeated `routine` cycles for doctrine just because the machine looked thoughtful while doing it.
