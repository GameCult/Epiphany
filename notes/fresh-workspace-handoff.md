...
- `epiphany-verse-query tool-directory` and `tools/epiphany_local_run.ps1 -Mode tool-directory` are the compact read-only daemon tool directory. The report reads the same local Verse capability records used by `invoke-tool`, annotates them with host daemon readiness, and returns TUI rows plus direct `invocationCommand` and wrapper `wrapperInvocationCommand=tools/epiphany_local_run.ps1 -Mode tool-invoke -ToolCapabilityId <capability> -ToolRequestingAgentId <agent> -ToolRequestingClusterId <cluster>`. TUI rows expose the global-tool contract directly, e.g. `READY | Hands | repo-action | submitHandsActionIntent | epiphany.cluster.hands.tool.repo-action | eve://epiphany/hands | authority=hands | input=epiphany.hands.action_intent | receiptType=epiphany.hands.action_review | allAgents=true | receipt=true | private=false`. Verified: fresh proof store returned seventeen globally available, receipt-gated, private-state-sealed tools with no host attention; smoke source-proves the compact Hands, Self `service-health`, and status readback rows plus command hints; wrapper proof `local-20260618-233719-320-e38a5549` printed all seventeen rows in the one-line `toolRows` summary with authority gates and advertised input/receipt contract types, including status tools, Self `service-health`, Hands `repo-action`, and Soul `verify`, with `privateStateExposed=False`.
...
- 2026-06-18: The daemon tool directory now advertises Self `service-health` as a globally available, receipt-gated, private-state-sealed CultMesh capability over the daemon service lifecycle contract. Source smoke reports `daemonTools=17`, and live wrapper proof `local-20260618-233719-320-e38a5549` printed `READY | Self | service-health | readServiceLifecycleStatus | epiphany.cluster.self.tool.service-health | eve://epiphany/self | authority=daemon.service_lifecycle | input=epiphany.cultmesh.daemon_service_lifecycle_query | receiptType=epiphany.cultmesh.daemon_service_lifecycle_receipt | allAgents=true | receipt=true | private=false`.
