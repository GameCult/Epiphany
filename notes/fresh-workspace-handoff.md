...
- Current Mom-ready status: the normal MVP pass reaches Face -> coordinator -> detached runtime -> heartbeat sleep, and the modeling worker can now complete through the ChatGPT subscription transport. The failure was a stack of violated observability/runtime invariants: worker prompts prepended the whole shared persistent-memory rite, Responses frame liveness was invisible until completion, and the runtime tried to persist hundreds of text deltas before completing the worker job. Role/reorient workers now get a compact Epiphany worker boundary instead of full persistent memory; runtime-spine records sampled raw Responses frame kinds with small previews; contiguous reasoning/text deltas compact before durable storage; and a 60-second live proof completed both `openai-worker-2fe38dcd-d99e-4c73-9a58-afcc81f851c7` and worker job `2fe38dcd-d99e-4c73-9a58-afcc81f851c7` with modeling verdict `regather-needed`. Next cut should run the status/acceptance path against that completed modeling result and verify the coordinator advances into verification rather than leaving late detached completion invisible.
...
- Epiphany prompt authority is now explicitly Codex-free. Bundled specialist prompts live in `epiphany-codex-bridge/src/prompts/epiphany_specialists.toml`, the app-server boundary has a regression test that fails if any bundled Epiphany agent prompt contains `Codex`, and `epiphany-repo-birth-runner` no longer accepts `codex-exec` or `--codex-bin`. Use `epiphany-model-runtime` for startup-only birth specialists so Codex cannot wrap Epiphany agents in its own prompt machinery.
...
`epiphany-codex-bridge/src/prompts/epiphany_specialists.toml`.
