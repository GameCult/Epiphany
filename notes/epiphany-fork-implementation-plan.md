...
- the richer app-server smoke now asserts direct update publishes source/revision/changed-fields `update`/`1`/`observations,evidence`, verifier-only accepted promotion publishes `promote`/`2`/`observations,evidence`, and accepted graph/churn promotion publishes `promote`/`3`/`graphs,graphFrontier,observations,evidence,churn`
