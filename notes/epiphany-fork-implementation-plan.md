...
- a direct-update integrity hardening slice: `thread/epiphany/update` now validates appended observation/evidence graph records before mutation, rejects empty/duplicate/reused append ids and missing evidence references, and the reusable smoke asserts invalid direct updates leave revision unchanged with no `thread/epiphany/stateUpdated` notification
...
- the richer app-server smoke now also asserts malformed direct `thread/epiphany/update` observation/evidence appends return an invalid-request error, leave revision at `2`, and leave `invalidDirectUpdateNotificationCount` at `0`
