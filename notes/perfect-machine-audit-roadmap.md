...
service desired/observed rows through provider-owned Eve without opening command
payloads or routine artifacts.
The first sight organ is landed as `epiphany-verse-query
managed-services`. Native process observation now lives in a shared
core module used by both Idunn reconciliation and provider sight projection. Rows show
...
routine payloads. Next lower this typed row family into the main provider-owned Eve
composition graph.
