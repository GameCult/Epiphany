...
`thread/epiphany/graphQuery` now prefers `state/memory-graph.msgpack` for
loaded threads and projects the typed repo memory graph back into the legacy
graph response shape only at the bridge edge.
