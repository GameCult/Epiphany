...
`privateStateExposed=false`.
The 2026-06-21 follow-up smoke
`.epiphany-smoke\repo-consensus-brief-family-20260621-005433` now also proves
that `epiphany-work overview` emits
`epiphany.repo_work_intake_consensus_readback.v0` and lowers it into the
repo-local CultMesh overview TUI rows as `CONSENSUS | ...`. The readback shows
route `imagination.consensus_discovery`, two candidate action refs, model-authored
safe family `repo.consensus_brief`, no Hands/durable-state/publication
authority, and `privateStateExposed=false`.
...
`.epiphany-smoke\repo-interpreter-brief-family-20260621-010858` ran init ->
online -> accept -> `derive-plan --action-family repo-interpreter-brief
--model-ref repo-interpreter-brief-family-smoke-imagination-v1 --model-authored`
-> tick through run/adopt/execute/close on a fresh repo. The plan carried
`safeActionFamily=repo.interpreter_brief`; Hands committed only
`.epiphany/interpreter-briefs/repo-interpreter-brief-family.toml`; Soul passed
with `pathScopeMatched=true` and `familyAssertions.status=passed`; the committed
brief carried `schema_version = "epiphany.repo_interpreter_brief.v0"`, draft
Mind ownership, Imagination as source, consensus/safe-family/path/verification /
evidence requirements, semantic checks for intent/scope/paths/verification /
evidence/rollback/non-goals/questions/consensus, candidate next safe families,
Mind/Soul/Bifrost/Hands/Substrate/Idunn gates, denied objective adoption,
scheduling, substrate access, Hands action, shell, commit, publication,
deployment execution, cross-body mutation, private transcript/raw-result access,
...
The next per-entry closure row makes the Soul/Modeling/Mind closure artery
visible without opening sealed receipt bodies:
`.epiphany-smoke\repo-close-mind-adoption-guard-20260621-075522` proves Gjallar
now derives `repoWorkMapClosureRows` and `REPO-WORK-MAP-CLOSURE` TUI rows from
Mind-admitted map entries. The proof row mirrors the accepted item, safe family,
branch, latest commit, changed-path count, Soul verdict receipt id, Mind gateway
review id, Mind state commit id, durable map admission, Bifrost publication
gate, `sightOnly=true`, and explicit denials for publication, merge, service
lifecycle, deployment execution, and cross-repo mutation while keeping
`privateStateExposed=false`; the smoke summary reports
`gjallarRepoWorkMapClosureCount=1` after counterfeit Mind adoption is still
refused. This is closure readback only, not publication authority, merge
...
`repoWorkGateLensRows` in the one-line operator summary. Follow-up wrapper proof
`local-20260621-085736-645-72c217a6` pointed `gjallar` at the closure-row smoke
repo-local Verse and printed `repoWorkClosureRows` with
`publicationAuth=false`, `mergeAuth=false`, `serviceAuth=false`,
`deploymentAuth=false`, `crossRepoAuth=false`, `sightOnly=true`, and
`private=false`. Earlier wrapper proofs
...
`epiphany.cultmesh.repo_work_readiness.v0` document with `REPO-WORK-READINESS`
TUI rows, missing proof kinds, readiness receipt ref, overview receipt ref, and
explicit sight-only authority denials. Gjallar, swarm overview, and swarm triage
now enumerate those documents as `repoWorkReadinessRows`, compact readiness TUI
rows, `latestRepoWorkReadiness`, and non-mutating `repo-work-readiness` action
...
`epiphany.repo_deployment_aftercare_audit.v0` artifact. The next
proof-bundle cut made Idunn and global tool sight receipt-shaped instead of
path-shaped: readiness now validates `.epiphany/work/repo-work-service-audit.json`
or `--idunn-lifecycle-receipt` as `epiphany.repo_work_service_audit.v0`,
requiring `status=complete`, Idunn ownership, hosted body `repo-work`, present
plan/runbook/artifact proof, `launchStatus=ok`, zero missing/failed checks, no
service-manager mutation, no elevation requirement, and
`privateStateExposed=false`; it also validates `.epiphany/work/tool-directory.json`
or `--tool-directory-receipt` as
`epiphany.cultmesh.daemon_tool_directory.v0`, requiring nonempty globally
available receipt-gated capabilities, ready hosts, typed direct/wrapper
invocation commands, and the private-state seal. It now validates
`bifrost-publication` from the local publish artifact plus matching repo-local
CultMesh `gamecult.bifrost.body_change_publication_receipt.v0` and
`gamecult.bifrost.github_publication_receipt.v0` rows, requiring matching
Bifrost/GitHub receipt ids, ledger id, Hands PR receipt id, PR URL, nonempty
credit and changed-path evidence, no upstream-main sync claim, no merge claim,
and no private-state exposure. The `upstream-main-sync` row now validates
`epiphany.repo_work_sync_receipt.v0` plus live `git merge-base --is-ancestor`
proof, requiring the published commit, upstream ref, upstream commit, nonempty
merge/sync authority receipts, Hands PR/commit ids, Bifrost/GitHub publication
ids, ledger id, and matching publish receipt. Native smoke
`.epiphany-smoke\repo-work-readiness-20260621-110524` now reports `ready` with
`missingRequiredCount=0`, `syncStatus=upstream-main-synced`,
`upstreamMainSynced=true`, readiness id `repo-work-readiness-repo-work-readiness`,
and Gjallar `repoWorkReadinessCount=1` without private-state exposure. Wrapper
proof `local-20260621-120641-609-72d32f64` prints the same no-gap readiness
mouth. This is still sight only: Bifrost/GitHub keep publication/sync ownership,
Idunn keeps lifecycle ownership, Odin/Gjallar keep discovery/sight, and no
readiness approval, publication, sync, service mutation, Hands action, or
private-state authority is granted.

`epiphany-work readiness-review` is now the explicit readiness adoption gate
after the sight receipt is ready. It consumes the ready
`epiphany.repo_work_readiness_report.v0`, requires four explicit review refs
from Maintainer, Soul, Mind, and Bifrost, refuses unless status is `ready`,
`missingRequiredCount=0`, every row is satisfied, the original readiness report
remained sight-only, and all dangerous authority stayed denied. It writes
`epiphany.repo_work_readiness_review_receipt.v0` with
`status=readiness-approved`, `readinessApprovalAuthorized=true`, and explicit
false authority for durable state commit, publication, PR, merge, upstream sync,
deployment, service lifecycle, Hands action, cross-body mutation, private Verse
rummaging, and private-state exposure. Native smoke
`.epiphany-smoke\repo-work-readiness-20260621-111411` proves the full ready
chain through the review receipt with sixteen satisfied rows. Wrapper proof
`local-20260621-121829-442-d011af09` invokes
`tools/epiphany_local_run.ps1 -Mode repo-work-readiness-review` and prints the
same approval boundary at the operator mouth.

The readiness-review adoption receipt is now indexed by the compact receipt
directory too. `epiphany-verse-query receipt-directory` reads the latest
`epiphany.cultmesh.repo_work_readiness_review.v0` row from local Verse and
publishes the `repo-work-readiness-review` family with owner
`Maintainer/Soul/Mind/Bifrost`, status `readiness-approved`, the readiness
receipt as route, the review receipt artifact ref/SHA-256, the follow-up wrapper
`tools/epiphany_local_run.ps1 -Mode repo-work-readiness-review`, and
`privateStateExposed=false`. The wrapper `receipt-directory` mode now respects
explicit `-LocalVerseStore` / `-LocalVerseRuntimeId` read targets for this row
while still writing operator run receipts to the operator store. Native proof
`.epiphany-smoke\repo-work-readiness-20260621-115547` reports
`receiptDirectoryReadinessReviewRow.family=repo-work-readiness-review`,
`latestId=repo-work-readiness-review-repo-work-readiness`,
`artifactStatus=present`, and SHA-256
`ba50bce3aa2af320a71977bde9d47108154665802dd527dd30bf8c4ab28bba5e`;
wrapper proof `local-20260621-125756-748-03710f48` prints the same row from the
repo-local Verse target. This is public-proof indexing only: it grants no
...
| Imagination planning | The catalog-shaped `repo.planning_brief` family is deleted because it had no candidate work-item records. Consensus Draft, Interpreter Brief request, and Objective Draft remain typed preparation cargo before Mind adoption. | Future plan artifacts must carry actual candidate records and reference separately owned catalog/contracts. |
