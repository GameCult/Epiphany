...
`epiphany.repo_work_readiness_report.v0`. The command is sight only: it denies
readiness approval, durable state commit, publication, PR, merge, sync,
deployment, service lifecycle, Hands action, cross-body mutation, private Verse
rummaging, and private-state exposure. Smoke
`.epiphany-smoke\repo-work-readiness-20260621-092053` proved a partial local
work path reports `not-ready` with four missing rows: Bifrost publication,
upstream-main sync, Idunn lifecycle, and tool directory.
