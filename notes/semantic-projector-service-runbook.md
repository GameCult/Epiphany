...
One `epiphany-memory-semantic-projector` process owns derived semantic indexing
for the Modeling projection of one Epiphany runtime:

- Modeling input: the configured runtime Mind CultCache store
- Derived index: the Modeling Qdrant collection
...
The process must remain beside its canonical runtime store. Yggdrasil may host
Ollama embedding work over WireGuard. Do not install a second projector for the
same runtime store.
...
1. Confirm the runtime Mind store exists and carries the admitted runtime/swarm identity.
...
4. Confirm no projector process already owns the canonical runtime store. The
...
Epiphany Modeling collection. The ops tunnel exposes its loopback
...
`restartMode=always`. Its generated child command contains the canonical
runtime store. The projector executable is resolved from the authenticated
...
projector holds the OS singleton and validates the canonical Modeling input.
...
- Read the Modeling semantic-health projection.
- Run a Modeling semantic query and confirm the query gate reports semantic
  ranking only when the newest exact obligation/success chain is authenticated.
...
& $semantic context --runtime-store .\state\runtime-spine.msgpack `
  --text 'current repository architecture and deployment frontier' `
  --query-id reboot-proof-modeling --budget 8
```

The result must report `semantic projection ranked ... canonical ...
