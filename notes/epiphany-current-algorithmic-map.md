...
The spine exists in sixteen live paths:
...
- freshness reflection: app-server declares experimental read-only `thread/epiphany/freshness`, reads the same live/stored thread view used by scene/jobs/context, fills live retrieval state for loaded threads, and derives one bounded retrieval/graph freshness lens with exact dirty-path pressure, checkpoint identity, churn hints, and revision/source identity without mutating, scheduling, notifying, or pretending watcher-driven invalidation already exists in [common.rs](E:/Projects/EpiphanyAgent/vendor/codex/codex-rs/app-server-protocol/src/protocol/common.rs:350), [v2.rs](E:/Projects/EpiphanyAgent/vendor/codex/codex-rs/app-server-protocol/src/protocol/v2.rs:4160), [codex_message_processor.rs](E:/Projects/EpiphanyAgent/vendor/codex/codex-rs/app-server/src/codex_message_processor.rs:4211), and [codex_message_processor.rs](E:/Projects/EpiphanyAgent/vendor/codex/codex-rs/app-server/src/codex_message_processor.rs:10892).
...
flowchart TD
    ...
    QJ --> Q
    N --> QF["thread/epiphany/freshness derives retrieval/graph freshness lens"]
    QF --> Q
...
- The app-server can expose a read-only freshness lens derived from retrieval summaries plus graph frontier/churn state without turning file staleness into a hidden worker.
...
The next Perfect Machine move is not adding another writer, not re-hardening Phase 5, and no longer the first scene, job, freshness, targeted context, pressure, or investigation-checkpoint reflection. Those surfaces now let clients inspect the machine Epiphany already built at summary, progress, bounded-detail, freshness, context-pressure, and durable-planning-packet levels. The next outward work should add a missing observation source only when there is a concrete consumer: live watcher-backed invalidation inputs, live progress notifications once real long-running job owners exist, or future CRRC coordination that consumes the landed checkpoint instead of inventing transcript telepathy.
...
| Freshness projection | Clients can ask whether retrieval or graph understanding is stale, missing, or ready, including exact dirty paths and checkpoint identity. The answer is a lens over existing retrieval/frontier/churn signals; it does not remap or invalidate anything by itself. | A clipboard listing which blueprint tabs got coffee on them and which shelf of the catalog needs rechecking, without sending anyone to swing a hammer. |
