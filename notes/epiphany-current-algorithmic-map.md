...
The spine exists in nineteen live paths:
...
flowchart TD
    ...
    QRLQ --> QRLA["Map checkpoint + reorient verdict into fixed reorient-specialist launch"]
    QRLA --> AJLA
...
-> can be consumed explicitly by one bounded reorient-specialist launch surface
...
- live app-server smoke for the first bounded reorient-launch runtime consumer in `tools/epiphany_phase6_reorient_launch_smoke.py`.
