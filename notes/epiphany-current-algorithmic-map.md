...
| model-provider connector | one configured caller's encrypted expiring native request over bounded loopback CultNet | ordered public model events, terminal receipt, and redacted CultMesh capability status | A consumer may borrow Codex transport and physical concurrency without borrowing Epiphany's Mind, credentials, swarm lifecycle, or state-admission authority. |
