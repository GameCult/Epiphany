...
- `repo.planning_brief` is condemned pending one coherent deletion pass: it contains no candidate work-item records and currently lets copied global catalog/schema/closure constants self-attest as per-item `safeFamilyPlanning`. The real preparation chain is consensus draft -> interpretation request -> objective draft -> Mind adoption.
