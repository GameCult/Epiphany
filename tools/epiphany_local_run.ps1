        ...
        if ($Mode -eq "smoke") {
            ...
            if ($null -ne $result.rows -and $result.rows.Count -gt 0) {
                $daemonRows = (($result.rows | ForEach-Object {
                    "$($_.displayName):$($_.status):$($_.daemonId):declaredPrivateVerseRoute=$($_.declaredPrivateVerseRoute):declaredEveRoute=$($_.declaredEveRoute)->$($result.wrapperMode)"
                }) -join "; ")
            }
            Write-Host "Swarm status: status=$($result.status), observedDaemons=$($result.observedDaemonCount), nonReady=$($result.nonReadyCount), daemonRows=$daemonRows, statusRows=$statusRows, privateStateExposed=$($result.privateStateExposed)"
        } elseif ($Mode -eq "cluster-topology") {
            $topologyRows = "none"
            if ($null -ne $result.tuiRows -and $result.tuiRows.Count -gt 0) {
                $topologyRows = ($result.tuiRows -join "; ")
            }
            Write-Host "Cluster topology: status=$($result.status), declaredFaculties=$($result.declaredFacultyCount), declaredPrivateVerseRoutes=$($result.declaredPrivateVerseRouteCount), declaredDaemonTargets=$($result.declaredDaemonTargetCount), publicDiscussion=$($result.publicDiscussionClusterCount), topologyRows=$topologyRows, privateStateExposed=$($result.privateStateExposed)"
            ...
            if ($null -ne $result.rows -and $result.rows.Count -gt 0) {
                $publicSurfaceRows = @($result.rows | Where-Object { $_.publicPersonaDiscussionAllowed -eq $true } | ForEach-Object {
                    "$($_.displayName)=$($_.eveSurfaceId)@declaredPrivateVerseRoute=$($_.declaredPrivateVerseRoute)"
                })
            ...
            Write-Host "Service policy directory: status=$($result.status), owner=$($result.lifecycleOwner), hostedBody=$($result.hostedBody), declaredDaemonTargets=$($result.declaredDaemonTargetCount), observedDaemons=$($result.observedDaemonCount), covered=$($result.coveredCount), enabled=$($result.enabledCount), disabled=$($result.disabledCount), missing=$($result.missingCount), attention=$($result.attentionCount), policyRows=$policyRows, privateStateExposed=$($result.privateStateExposed)"
