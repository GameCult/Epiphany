param(
    ...
    [string]$BifrostArtifactSummaryPath = "",
    [string]$BifrostMetricsSummaryPath = "",
    ...
    [string]$RepoWorkMaintainerReviewReceipt = "maintainer-review:local-proof",
    [string]$RepoWorkSoulReviewReceipt = "soul-review:local-proof",
    [string]$RepoWorkMindReviewReceipt = "mind-review:local-proof",
    [string]$RepoWorkBifrostReviewReceipt = "bifrost-review:local-proof",
    [string]$RepoWorkReadinessReviewSummary = "Operator-safe local readiness review approves a ready sight receipt without granting action authority.",
...
$repoSwarmMvpGateExe = Join-Path $TargetDir "debug\epiphany-repo-swarm-mvp-gate-smoke.exe"
$repoLivefireClosureSmokeExe = Join-Path $TargetDir "debug\epiphany-repo-livefire-closure-smoke.exe"
$bifrostAccountingBundleExe = Join-Path $TargetDir "debug\epiphany-repo-bifrost-accounting-bundle-smoke.exe"
...
if (-not $SkipBuild) {
    ...
    Invoke-Checked `
        ...
        -Arguments @(
            ...
            "--bin", "epiphany-repo-swarm-mvp-gate-smoke",
            "--bin", "epiphany-repo-livefire-closure-smoke",
            "--bin", "epiphany-repo-bifrost-accounting-bundle-smoke",
            "--bin", "epiphany-swarm",
            "--bin", "epiphany-work",
...
if ($Mode -eq "repo-swarm-mvp-gate") {
    $requiredBinaries += @($repoSwarmMvpGateExe)
}
...
if (@("plan", "run", "mvp", "repo-persona-intake", "repo-swarm-run", "repo-work-queue-run", "repo-livefire-closure", "repo-work-public-proof", "repo-work-readiness", "repo-work-readiness-review", "repo-deployment-config-audit", "repo-deployment-runbook", "repo-deployment-aftercare-audit", "repo-work-service-plan", "repo-work-service-runbook", "repo-work-service-launch", "repo-work-service-audit", "service-plan", "service-launch", "service-runbook", "cluster-service-runbook", "cluster-service-install-plan", "cluster-service-install-execute", "cluster-service-audit", "cluster-service-start-plan", "cluster-service-stop-plan", "cluster-service-start-execute", "cluster-service-stop-execute", "cluster-service-execution-readiness", "cluster-service-execution-runbook", "cluster-service-execution-audit", "service-execution-runbook", "service-install-plan", "service-install-execute", "service-tick", "service-status", "service-reconcile", "service-execution-readiness", "service-execution-audit", "service-start-plan", "service-stop-plan", "service-start-execute", "service-stop-execute") -contains $Mode) {
...
if ($Mode -eq "repo-swarm-mvp-gate") {
    $resultPath = Join-Path $artifactRoot "repo-swarm-mvp-gate.stdout.json"
    $gateArgs = @(
        "--root", $Root,
        "--smoke-root", (Join-Path $Root ".epiphany-smoke")
    )
    if ($RepoSwarmMvpFreshRepoSummaryPath -ne "") {
        $gateArgs += @("--fresh-repo-summary", $RepoSwarmMvpFreshRepoSummaryPath)
    }
    if ($RepoSwarmMvpReadinessSummaryPath -ne "") {
        $gateArgs += @("--readiness-summary", $RepoSwarmMvpReadinessSummaryPath)
    }
    ...
    Invoke-Checked `
        -Label "verify repo swarm MVP gate proof" `
        -FilePath $repoSwarmMvpGateExe `
        -Arguments $gateArgs `
        -WorkingDirectory $Root `
        -StdoutPath $resultPath `
        -StderrPath (Join-Path $artifactRoot "repo-swarm-mvp-gate.stderr.log")
}
...
if ($Mode -eq "bifrost-accounting-bundle") {
    $resultPath = Join-Path $artifactRoot "bifrost-accounting-bundle.stdout.json"
    $bundleArgs = @(
        "--root", $Root,
        "--smoke-root", (Join-Path $Root ".epiphany-smoke")
    )
    if ($BifrostArtifactSummaryPath -ne "") {
        $bundleArgs += @("--artifact-summary", $BifrostArtifactSummaryPath)
    }
    if ($BifrostMetricsSummaryPath -ne "") {
        $bundleArgs += @("--metrics-summary", $BifrostMetricsSummaryPath)
    }
    Invoke-Checked `
        -Label "verify Bifrost accounting bundle proof" `
        -FilePath $bifrostAccountingBundleExe `
        -Arguments $bundleArgs `
        -WorkingDirectory $Root `
        -StdoutPath $resultPath `
        -StderrPath (Join-Path $artifactRoot "bifrost-accounting-bundle.stderr.log")
}
...
if ($Mode -eq "repo-work-readiness-review") {
    $resultPath = Join-Path $artifactRoot "repo-work-readiness-review.stdout.json"
    $repoWorkArgs = @(
        "readiness-review",
        "--workspace", $Workspace,
        "--item", $RepoWorkItem,
        "--maintainer-review-receipt", $RepoWorkMaintainerReviewReceipt,
        "--soul-review-receipt", $RepoWorkSoulReviewReceipt,
        "--mind-review-receipt", $RepoWorkMindReviewReceipt,
        "--bifrost-review-receipt", $RepoWorkBifrostReviewReceipt,
        "--review-summary", $RepoWorkReadinessReviewSummary
    )
    if ($RepoWorkReadinessReceipt -ne "") {
        $repoWorkArgs += @("--readiness-receipt", $RepoWorkReadinessReceipt)
    }
    Invoke-Checked `
        -Label "review repo-work MVP readiness sight" `
        -FilePath $repoWorkExe `
        -Arguments $repoWorkArgs `
        -WorkingDirectory $Root `
        -StdoutPath $resultPath `
        -StderrPath (Join-Path $artifactRoot "repo-work-readiness-review.stderr.log")
}
...
if ($resultPath -ne "" -and (Test-Path -LiteralPath $resultPath)) {
    try {
        ...
        if ($Mode -eq "smoke") {
            ...
            $remaining = "none"
            if ($null -ne $result.knownRemainingAuthorityGates -and $result.knownRemainingAuthorityGates.Count -gt 0) {
                $remaining = ($result.knownRemainingAuthorityGates -join "; ")
            }
            Write-Host "Repo swarm MVP gate: status=$($result.status), demoReady=$($result.demoReady), mvpReady=$($result.mvpReady), green=$($result.greenGateCount), warnings=$($result.warningGateCount), blockers=$($result.blockerGateCount), remaining='$remaining', privateStateExposed=$($result.privateStateExposed), smokeDir=$($result.smokeDir)"
        ...
        } elseif ($Mode -eq "bifrost-accounting-bundle") {
            Write-Host "Bifrost accounting bundle: status=$($result.status), artifactFamily=$($result.artifactAcceptanceFamily), metricsFamily=$($result.metricsFamily), artifactReceipt=$($result.artifactAcceptanceReceiptId), metricsReceipt=$($result.metricsReceiptId), planningAuthorityOnly=$($result.planningAuthorityOnly), operatorSafeProofBundle=$($result.operatorSafeProofBundle), privateStateExposed=$($result.privateStateExposed), smokeDir=$($result.smokeDir)"
        ...
        } elseif ($Mode -eq "repo-work-readiness-review") {
            Write-Host "Repo work readiness review: status=$($result.status), item=$($result.item), review=$($result.reviewId), missing=$($result.missingRequiredCount), satisfied=$($result.satisfiedRowCount), receipt=$($result.receiptPath), readiness=$($result.readinessReceiptPath), readinessApprovalAuthorized=$($result.authority.readinessApprovalAuthorized), durableStateCommitAuthorized=$($result.authority.durableStateCommitAuthorized), publicationAuthorized=$($result.authority.publicationAuthorized), mergeAuthorized=$($result.authority.mergeAuthorized), serviceLifecycleAuthority=$($result.authority.serviceLifecycleAuthority), handsActionAuthorized=$($result.authority.handsActionAuthorized), next=$($result.nextSafeMove), privateStateExposed=$($result.privateStateExposed)"
