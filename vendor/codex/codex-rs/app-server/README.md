...
- `thread/epiphany/freshness` is a read-only reflection surface for retrieval and graph staleness. It reports revision/source identity, retrieval freshness, graph freshness, and exact dirty-path pressure without mutating state, scheduling refresh work, or pretending watcher-driven invalidation already exists.
