...
- `thread/epiphany/jobsUpdated` is an experimental server notification emitted from live `agent_job_progress:{json}` background events when a runtime `agent_jobs` owner is bound through durable `jobBindings` and the mapped bound-job payload actually changes. It carries the current `stateRevision` plus the changed bound job snapshots; it does not turn `thread/epiphany/jobs` into a writer or scheduler.
