...
#[derive(Clone, Debug, PartialEq, Eq, DatabaseEntry)]
#[cultcache(
    type = "epiphany.coordinator_run_basis.v0",
    schema = "EpiphanyCoordinatorRunBasis"
)]
pub struct EpiphanyCoordinatorRunBasis {
    #[cultcache(key = 1)]
    pub session_id: String,
    #[cultcache(key = 2)]
        ...
        match &decision.role_result {
            Some(result)
                if result.job_id == self.job_id
                    ...
                    && (fulfilled == result.item_error.is_none()) =>
            {
                Ok(())
            }
...
fn worker_outcome_status(
    item_error: Option<&str>,
) -> Result<(EpiphanyRuntimeJobStatus, WorkerProcessStatus)> {
    match item_error {
        Some(error) if error.trim().is_empty() => {
            Err(anyhow!("typed worker failure requires a nonempty error"))
        }
        Some(_) => Ok((
            EpiphanyRuntimeJobStatus::Failed,
            WorkerProcessStatus::TerminalFailure,
        )),
        None => Ok((
            EpiphanyRuntimeJobStatus::Completed,
            WorkerProcessStatus::TerminalResult,
        )),
    }
}

fn process_status_for_terminal_job(
    status: &EpiphanyRuntimeJobStatus,
) -> Result<WorkerProcessStatus> {
    match status {
        EpiphanyRuntimeJobStatus::Completed => Ok(WorkerProcessStatus::TerminalResult),
        EpiphanyRuntimeJobStatus::Failed => Ok(WorkerProcessStatus::TerminalFailure),
        EpiphanyRuntimeJobStatus::Queued => {
            Err(anyhow!("runtime job terminal status cannot be queued"))
        }
    }
}
...
#[derive(Clone, Debug, PartialEq, Eq, DatabaseEntry)]
#[cultcache(
    type = "epiphany.coordinator_run_terminality.v0",
    schema = "EpiphanyCoordinatorRunTerminality"
)]
pub struct EpiphanyCoordinatorRunTerminality {
    #[cultcache(key = 1)]
...
fn runtime_spine_schema_cache() -> Result<CultCache> {
    ...
    cache.register_entry_type::<EpiphanyCoordinatorRunBasis>()?;
    cache.register_entry_type::<EpiphanyCoordinatorRunTerminality>()?;
...
pub fn open_runtime_model_execution(
    ...
    cache.pull_all_backing_stores()?;
    require_identity(&cache)?;
    require_runtime_identity_not_archived(&cache, "session", &job_options.session_id)?;
    ...
    let existing_session = cache.get::<EpiphanyRuntimeSession>(&job_options.session_id)?;
    let session = existing_session
        .clone()
        .unwrap_or_else(|| EpiphanyRuntimeSession {
            session_id: job_options.session_id.clone(),
...
fn archive_completed_model_session(
    ...
    if cache
        ...
        || cache
            ...
            .any(|item| job_ids.contains(&item.job_id))
        || cache
            .get::<EpiphanyCoordinatorRunBasis>(session_id)?
            .is_some()
        || cache
    ...
    for native_request in &native_requests {
        ...
        match &native_terminals[0].payload {
            EpiphanyModelStreamPayload::Completed { receipt } => {
                if cache.get::<EpiphanyModelReceipt>(request_id)?.as_ref() != Some(receipt.as_ref())
                {
                    return Err(anyhow!(
...
pub fn runtime_job(
    store_path: impl AsRef<Path>,
    job_id: &str,
) -> Result<Option<EpiphanyRuntimeJob>> {
    ...
    let mut cache = runtime_spine_cache(store_path)?;
    cache.pull_all_backing_stores()?;
    cache.get::<EpiphanyRuntimeJob>(job_id)
}
...
pub(crate) fn terminalize_dead_runtime_worker_attempt(
    ...
    if role.is_some() || reorient.is_some() {
        let job = cache
            .get::<EpiphanyRuntimeJob>(job_id)?
            .ok_or_else(|| anyhow!("structured worker outcome lost its runtime job"))?;
        let claim = cache
            .get::<EpiphanyRuntimeWorkerProcessClaim>(&worker_process_claim_id(job_id))?
            .ok_or_else(|| anyhow!("structured worker outcome lost its process claim"))?;
        let result = role
            .as_ref()
            .map(|result| (&result.result_id, result.item_error.as_deref()))
            .or_else(|| {
                reorient
                    .as_ref()
                    .map(|result| (&result.result_id, result.item_error.as_deref()))
            })
            .expect("one structured outcome checked above");
        let (expected_job_status, expected_process_status) = worker_outcome_status(result.1)?;
        if job.status != expected_job_status
            || WorkerProcessStatus::parse(&claim.status)? != expected_process_status
            || claim.terminal_authority_id.as_deref() != Some(result.0.as_str())
        {
            return Err(anyhow!(
                "structured worker outcome did not atomically terminalize its physiology"
            ));
        }
        return Ok(job);
    }
...
pub fn put_runtime_role_worker_result(
    ...
    let (terminal_job_status, terminal_process_status) =
        worker_outcome_status(result.item_error.as_deref())?;
    let is_failure = terminal_job_status == EpiphanyRuntimeJobStatus::Failed;
    ...
    let mut job = cache
        .get::<EpiphanyRuntimeJob>(&result.job_id)?
        .ok_or_else(|| anyhow!("role worker result requires its runtime job"))?;
    if let Some(existing) = cache.get::<EpiphanyRuntimeRoleWorkerResult>(&result.job_id)? {
        ...
        if job.status != terminal_job_status {
            return Err(anyhow!(
                "role worker result is not terminal authority for its runtime job"
            ));
        }
        if let Some(claim) = process_claim.as_ref()
            && (crate::WorkerProcessStatus::parse(&claim.status)? != terminal_process_status
    ...
    if job.status != EpiphanyRuntimeJobStatus::Queued {
        return Err(anyhow!(
            "role worker result requires its queued runtime job"
        ));
    }
    let job_envelope = cache
        .get_envelope::<EpiphanyRuntimeJob>(&result.job_id)?
        .ok_or_else(|| anyhow!("role worker result lost its job envelope"))?;
    job.status = terminal_job_status.clone();
    let (envelope, _) = cache.prepare_entry(&result.job_id, result)?;
    let mut writes = vec![envelope, cache.prepare_entry(&result.job_id, &job)?.0];
    ...
    if let Some(claim) = process_claim.as_ref() {
        ...
        terminal_claim.status = terminal_process_status.as_str().into();
        ...
        Some(_) => reloaded
            ...
            .is_some_and(|claim| {
                crate::WorkerProcessStatus::parse(&claim.status)
                    .is_ok_and(|status| status == terminal_process_status)
                    && claim.terminal_authority_id.as_deref() == Some(result.result_id.as_str())
            }),
        None => true,
    };
    let job_matches = reloaded
        .get::<EpiphanyRuntimeJob>(&result.job_id)?
        .is_some_and(|job| job.status == terminal_job_status);
    if worker_matches
        && job_matches
        ...
        match job.status {
            ...
            EpiphanyRuntimeJobStatus::Failed => {
                let claim = cache
                    ...
                    .ok_or_else(|| {
                        anyhow!("failed frontier Research attempt lost its process claim")
                    })?;
                let claim_status = WorkerProcessStatus::parse(&claim.status)?;
                if !matches!(
                    claim_status,
                    WorkerProcessStatus::TerminalResult
                        | WorkerProcessStatus::TerminalFailure
                        | WorkerProcessStatus::TerminalDeath
                ) || claim.terminal_authority_id.is_none()
                {
                    return Err(anyhow!(
                        "failed frontier Research attempt lost its exact terminal authority"
                    ));
                }
            }
...
pub fn put_runtime_reorient_worker_result(
    ...
    let launch = cache
        .get::<EpiphanyRuntimeWorkerLaunchRequest>(&result.job_id)?
        .ok_or_else(|| anyhow!("reorientation result requires its immutable launch"))?;
    if !matches!(
        launch.launch_document()?,
        EpiphanyWorkerLaunchDocument::Reorient(_)
    ) {
        return Err(anyhow!("reorientation result crossed a role launch"));
    }
    let (terminal_job_status, terminal_process_status) =
        worker_outcome_status(result.item_error.as_deref())?;
    let mut job = cache
        .get::<EpiphanyRuntimeJob>(&result.job_id)?
        .ok_or_else(|| anyhow!("reorientation result requires its runtime job"))?;
    let claim_id = worker_process_claim_id(&result.job_id);
    let claim = cache.get::<EpiphanyRuntimeWorkerProcessClaim>(&claim_id)?;
    if let Some(existing) = cache.get::<EpiphanyRuntimeReorientWorkerResult>(&result.job_id)? {
        if existing != *result
            || job.status != terminal_job_status
            || claim.as_ref().is_some_and(|claim| {
                WorkerProcessStatus::parse(&claim.status).ok() != Some(terminal_process_status)
                    || claim.terminal_authority_id.as_deref() != Some(result.result_id.as_str())
            })
        {
            return Err(anyhow!("reorientation result replay was substituted"));
        }
        return Ok(());
    }
    if job.status != EpiphanyRuntimeJobStatus::Queued {
        return Err(anyhow!(
            "reorientation result requires its queued runtime job"
        ));
    }
    let job_envelope = cache
        .get_envelope::<EpiphanyRuntimeJob>(&result.job_id)?
        .ok_or_else(|| anyhow!("reorientation result lost its job envelope"))?;
    job.status = terminal_job_status.clone();
    let mut expected = vec![job_envelope];
    let mut writes = vec![
        cache.prepare_entry(&result.job_id, result)?.0,
        cache.prepare_entry(&result.job_id, &job)?.0,
    ];
    if let Some(claim) = claim {
        if WorkerProcessStatus::parse(&claim.status)? != WorkerProcessStatus::Active {
            return Err(anyhow!(
                "reorientation result requires its active process claim"
            ));
        }
        let claim_envelope = cache
            .get_envelope::<EpiphanyRuntimeWorkerProcessClaim>(&claim_id)?
            .ok_or_else(|| anyhow!("reorientation result lost its claim envelope"))?;
        let mut terminal = claim;
        terminal.status = terminal_process_status.as_str().into();
        terminal.terminal_at = Some(chrono::Utc::now().to_rfc3339());
    ...
    reloaded.pull_all_backing_stores()?;
    if reloaded
        .get::<EpiphanyRuntimeReorientWorkerResult>(&result.job_id)?
        .is_some_and(|existing| existing == *result)
        && reloaded
            .get::<EpiphanyRuntimeJob>(&result.job_id)?
            .is_some_and(|job| job.status == terminal_job_status)
            ...
            })
    {
        ...
        Err(anyhow!(
            "reorientation result lost immutable insertion to a different outcome"
        ))
    }
}
...
fn validate_coordinator_run_receipt(receipt: &EpiphanyCoordinatorRunReceipt) -> Result<()> {
    ...
    if receipt.receipt_id != format!("coordinator-run-terminal-{}", receipt.session_id) {
        return Err(anyhow!(
            "coordinator run receipt identity is not its session's terminal identity"
        ));
    }
    Ok(())
}

fn coordinator_run_terminality<T: Serialize>(
    session_id: &str,
    terminal_kind: &str,
    authority_id: &str,
    authority: &T,
) -> Result<EpiphanyCoordinatorRunTerminality> {
    let payload = rmp_serde::to_vec_named(authority)
        .context("failed to encode coordinator terminal authority")?;
    Ok(EpiphanyCoordinatorRunTerminality {
        terminal_id: format!("coordinator-run-terminal-{session_id}"),
        session_id: session_id.to_string(),
        terminal_kind: terminal_kind.to_string(),
        authority_id: authority_id.to_string(),
        authority_sha256: format!("sha256:{:x}", Sha256::digest(payload)),
    })
}
...
fn archive_runtime_worker_attempt(
    ...
    if fulfilled && role_result.is_none() {
        return Err(anyhow!(
    ...
    if role_result
        .as_ref()
        .is_some_and(|result| fulfilled == result.item_error.is_some())
    {
        return Err(anyhow!(
            "worker attempt archive structured result disagrees with process terminality"
        ));
    }
    let claim_id = worker_process_claim_id(job_id);
    let claim = cache
    ...
    let mut decision_context_ids = cache
        .get_all::<crate::EpiphanyModelPassFailure>()?
        .iter()
        .filter(|failure| failure.pass_id == job_id)
        .map(|failure| failure.decision_context_id.clone())
        ...
        .filter(|entry| {
            (entry.r#type == EpiphanyRuntimeWorkerLaunchRequest::TYPE && entry.key == job_id)
                || (entry.r#type == EpiphanyRuntimeWorkerProcessClaim::TYPE
                    && entry.key == claim_id)
                || (entry.r#type == EpiphanyRuntimeRoleWorkerResult::TYPE && entry.key == job_id)
...
pub fn open_coordinator_run(
    ...
    objective: &str,
    started_at: &str,
) -> Result<EpiphanyCoordinatorRunBasis> {
    ...
    let basis = EpiphanyCoordinatorRunBasis {
        session_id: session_id.to_string(),
        thread_id: thread_id.to_string(),
        resident_launch_digest: resident_launch_digest.map(str::to_string),
        objective: objective.to_string(),
        started_at: started_at.to_string(),
    };
    let envelope = cache.prepare_entry(session_id, &basis)?.0;
    if !runtime_spine_backing_store(store_path)?.compare_and_swap_batch(&[], vec![envelope])? {
        return Err(anyhow!("coordinator run opening lost immutable admission"));
    }
    Ok(basis)
}
...
pub(crate) fn recover_coordinator_run_after_exact_process_death(
    ...
    cache.pull_all_backing_stores()?;
    require_identity(&cache)?;
    let terminality = coordinator_run_terminality(
        &recovery.session_id,
        "death-recovery",
        &recovery.recovery_id,
        recovery,
    )?;
    if let Some(existing) = cache.get::<EpiphanyCoordinatorRunTerminality>(&recovery.session_id)? {
        if existing == terminality
            && cache.get::<EpiphanyCoordinatorDeathRecovery>(&recovery.recovery_id)?
                == Some(recovery.clone())
        {
            return Ok(());
        }
        return Err(anyhow!("coordinator terminal authority conflict"));
    }
    let basis = cache
        .get::<EpiphanyCoordinatorRunBasis>(&recovery.session_id)?
        .ok_or_else(|| anyhow!("coordinator death recovery basis is absent"))?;
    if basis.objective != expected_objective
        || basis.thread_id != recovery.thread_id
        || basis.resident_launch_digest.as_deref() != Some(&recovery.resident_launch_digest)
    {
        return Err(anyhow!(
            "coordinator death recovery requires the exact run basis"
        ));
    }
    let session_started_at = chrono::DateTime::parse_from_rfc3339(&basis.started_at)
    ...
    let basis_envelope =
        cache.get_required_envelope::<EpiphanyCoordinatorRunBasis>(&recovery.session_id)?;
    let replacements = vec![
        basis_envelope.clone(),
        cache
            .prepare_entry(&terminality.session_id, &terminality)?
            .0,
        cache.prepare_entry(&recovery.recovery_id, recovery)?.0,
    ];
    if !runtime_spine_backing_store(store_path)?
        .compare_and_swap_batch(&[basis_envelope], replacements)?
    {
        return Err(anyhow!("coordinator death recovery lost exact admission"));
    }
    Ok(())
}
...
pub fn finalize_coordinator_run(
    ...
    require_identity(&cache)?;
    let terminality =
        coordinator_run_terminality(&receipt.session_id, "receipt", &receipt.receipt_id, receipt)?;
    if let Some(existing) = cache.get::<EpiphanyCoordinatorRunTerminality>(&receipt.session_id)? {
        if existing == terminality
    ...
    let basis = cache
        .get::<EpiphanyCoordinatorRunBasis>(&receipt.session_id)?
        .ok_or_else(|| {
            anyhow!(
                "coordinator run basis {:?} does not exist",
                receipt.session_id
            )
        })?;
    if basis.thread_id != receipt.thread_id
        || basis.resident_launch_digest != receipt.resident_launch_digest
    {
    ...
    let basis_envelope =
        cache.get_required_envelope::<EpiphanyCoordinatorRunBasis>(&receipt.session_id)?;
    let replacements = vec![
        basis_envelope.clone(),
        cache
            .prepare_entry(&terminality.session_id, &terminality)?
    ...
    if !runtime_spine_backing_store(store_path)?
        .compare_and_swap_batch(&[basis_envelope], replacements)?
    {
        return Err(anyhow!("coordinator run finalization lost exact admission"));
    }
    Ok(())
}
...
pub fn terminalize_runtime_job(
    store_path: impl AsRef<Path>,
    job_id: &str,
    status: EpiphanyRuntimeJobStatus,
    terminal_at: &str,
    ...
    validate_non_empty(job_id, "job id")?;
    validate_non_empty(terminal_authority_id, "terminal authority id")?;
    chrono::DateTime::parse_from_rfc3339(terminal_at)
        .map_err(|error| anyhow!("runtime job terminal time is invalid: {error}"))?;
    if status == EpiphanyRuntimeJobStatus::Queued {
        return Err(anyhow!("runtime job terminal status cannot be queued"));
    }
    let store_path = store_path.as_ref();
    let mut cache = runtime_spine_cache(store_path)?;
    cache.pull_all_backing_stores()?;
    ...
    {
        return Err(anyhow!(
            "structured worker outcomes terminalize their own runtime job"
        ));
    }
    let mut job = cache
        .get::<EpiphanyRuntimeJob>(job_id)?
        .ok_or_else(|| anyhow!("runtime job {job_id:?} does not exist"))?;
    let claim_id = worker_process_claim_id(job_id);
    let claim = cache.get::<EpiphanyRuntimeWorkerProcessClaim>(&claim_id)?;
    if job.status != EpiphanyRuntimeJobStatus::Queued {
        if job.status != status {
            return Err(anyhow!("runtime job terminal status was substituted"));
        }
        if let Some(claim) = claim {
            let expected_claim_status = process_status_for_terminal_job(&status)?;
            if WorkerProcessStatus::parse(&claim.status)? != expected_claim_status
                || claim.terminal_authority_id.as_deref() != Some(terminal_authority_id)
            {
                return Err(anyhow!("runtime job replay crossed its process authority"));
            }
        }
        return Ok(job);
    }
    ...
    let mut expected = vec![job_envelope];
    let mut writes = vec![cache.prepare_entry(job_id, &job)?.0];
    if let Some(claim) = claim {
        if WorkerProcessStatus::parse(&claim.status)? != WorkerProcessStatus::Active {
            return Err(anyhow!(
                "runtime worker job cannot terminalize without its active process claim"
            ));
        }
        let claim_envelope = cache
            .get_envelope::<EpiphanyRuntimeWorkerProcessClaim>(&claim_id)?
            .ok_or_else(|| anyhow!("runtime worker terminality lost its claim envelope"))?;
        let mut terminal = claim;
        terminal.status = process_status_for_terminal_job(&status)?.as_str().into();
        terminal.terminal_at = Some(terminal_at.to_string());
        terminal.terminal_authority_id = Some(terminal_authority_id.to_string());
        expected.push(claim_envelope);
        writes.push(cache.prepare_entry(&claim_id, &terminal)?.0);
    }
    if runtime_spine_backing_store(store_path)?.compare_and_swap_batch(&expected, writes)? {
        return Ok(job);
    }
...
pub(crate) mod tests {
    ...
    #[test]
    fn structured_worker_failure_cannot_claim_fulfillment() -> Result<()> {
        assert_eq!(
            worker_outcome_status(None)?,
            (
                EpiphanyRuntimeJobStatus::Completed,
                WorkerProcessStatus::TerminalResult,
            )
        ...
        assert_eq!(
            worker_outcome_status(Some("provider refused"))?,
            (
                EpiphanyRuntimeJobStatus::Failed,
                WorkerProcessStatus::TerminalFailure,
            )
        );
        assert!(worker_outcome_status(Some(" ")).is_err());
        Ok(())
    }
    ...
    fn coordinator_terminal_receipts_preserve_exact_immutable_run_basis() -> Result<()> {
        let temp = tempfile::tempdir()?;
        let completed_store = temp.path().join("completed.cc");
        initialize_runtime_spine(
            &completed_store,
            RuntimeSpineInitOptions {
                runtime_id: "coordinator-terminal-state".into(),
                display_name: "Coordinator terminal state test".into(),
                created_at: "2026-08-24T00:00:00Z".into(),
            },
        )?;
        let session_id = coordinator_run_session_id("thread-completed", None)?;
        open_coordinator_run(
            &completed_store,
            &session_id,
            "thread-completed",
            None,
            "Complete one coordinator run.",
        ...
        let receipt = EpiphanyCoordinatorRunReceipt {
            ...
            final_action: "stop".into(),
            final_reason: None,
            created_at: "2026-08-24T00:00:02Z".into(),
        ...
        finalize_coordinator_run(&completed_store, &receipt)?;
        finalize_coordinator_run(&completed_store, &receipt)?;
        let mut cache = runtime_spine_cache(&completed_store)?;
        cache.pull_all_backing_stores()?;
        assert!(
            cache
                .get::<EpiphanyCoordinatorRunBasis>(&session_id)?
        ...
        assert!(cache.get::<EpiphanyRuntimeSession>(&session_id)?.is_none());
        assert_eq!(
            cache.get::<EpiphanyCoordinatorRunReceipt>(&receipt.receipt_id)?,
            Some(receipt.clone())
        );
        assert_eq!(
            cache
                .get::<EpiphanyCoordinatorRunTerminality>(&session_id)?
                .expect("completed run should have one terminality owner")
                .authority_id,
            receipt.receipt_id
        );
        let mut competing_receipt = receipt;
        competing_receipt.status = "failed".into();
        assert!(finalize_coordinator_run(&completed_store, &competing_receipt).is_err());

        let recovered_store = temp.path().join("recovered.cc");
        initialize_runtime_spine(
            &recovered_store,
            RuntimeSpineInitOptions {
                runtime_id: "coordinator-recovery-state".into(),
                display_name: "Coordinator recovery state test".into(),
                created_at: "2026-08-24T00:00:00Z".into(),
            },
        )?;
        let digest = format!("sha256:{}", "a".repeat(64));
        let recovered_session_id = coordinator_run_session_id("thread-recovered", Some(&digest))?;
        let started_at = "2026-08-24T00:00:03Z";
        open_coordinator_run(
            &recovered_store,
            &recovered_session_id,
            "thread-recovered",
            Some(&digest),
            "Resident coordinator launch objective-digest.",
            started_at,
        )?;
        let recovery = EpiphanyCoordinatorDeathRecovery {
            recovery_id: format!("coordinator-death-recovery-{recovered_session_id}"),
            session_id: recovered_session_id.clone(),
            thread_id: "thread-recovered".into(),
            resident_grant_id: "grant-1".into(),
            resident_launch_digest: digest,
            process_id: 42,
            process_creation_token: 7,
            process_executable_path: "F:\\Epiphany\\epiphany-mvp-coordinator.exe".into(),
            resident_started_at_millis: chrono::DateTime::parse_from_rfc3339(started_at)?
                .timestamp_millis() as u64,
            observation: "missing".into(),
            recovered_at: "2026-08-24T00:00:04Z".into(),
            private_state_exposed: false,
            exit_code: None,
        };
        recover_coordinator_run_after_exact_process_death(
            &recovered_store,
            &recovery,
            "Resident coordinator launch objective-digest.",
        )?;
        ...
        let mut cache = runtime_spine_cache(&recovered_store)?;
        cache.pull_all_backing_stores()?;
        assert!(
            cache
                .get::<EpiphanyCoordinatorRunBasis>(&recovered_session_id)?
                .is_some()
        );
        assert_eq!(
            cache.get::<EpiphanyCoordinatorDeathRecovery>(&recovery.recovery_id)?,
        ...
        assert_eq!(
            cache
                .get::<EpiphanyCoordinatorRunTerminality>(&recovered_session_id)?
                .expect("recovered run should have one terminality owner")
                .authority_id,
            recovery.recovery_id
        );
        let competing_receipt = EpiphanyCoordinatorRunReceipt {
            receipt_id: format!("coordinator-run-terminal-{recovered_session_id}"),
            ...
            final_action: "stop".into(),
            final_reason: None,
            created_at: "2026-08-24T00:00:05Z".into(),
            resident_grant_id: Some("grant-1".into()),
            resident_launch_digest: Some(recovery.resident_launch_digest),
            resident_policy_digest: None,
            resident_argv_digest: None,
            resident_objective_digest: None,
            resident_release_commit: None,
        ...
        assert!(finalize_coordinator_run(&recovered_store, &competing_receipt).is_err());
        Ok(())
    }
}
