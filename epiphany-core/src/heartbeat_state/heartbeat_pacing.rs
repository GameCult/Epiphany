...
pub(super) fn adaptive_swarm_pacing(
    ...
    let pressure = [external_urgency, pending_pressure * 0.65]
        .into_iter()
    ...
    AdaptiveSwarmPacing {
        ...
        signals: HeartbeatAdaptivePacingSignals {
            ...
            contract: "Explicit external urgency and running-turn pressure determine scheduler tempo and concurrency.".to_string(),
        },
    }
}
...
pub(super) fn effective_cooldown_multiplier(participant: &HeartbeatParticipant) -> f64 {
    round3(1.0 / initiative_heat_multiplier(participant)).clamp(0.05, 8.0)
}
