...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-manifest-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Manifest Family Smoke\n\nThis repository proves branch-local Epiphany Body manifests.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(["commit", "-m", "Seed repo manifest smoke body"], &repo)?;
    git(["switch", "-c", "epiphany/repo-manifest-family"], &repo)?;

    let item = "repo-manifest-family";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Publish a branch-local Epiphany Body manifest with private, local, public Verse ids and an Eve surface.",
            "--candidate-action-ref",
            "candidate-action://repo-manifest-family/body-manifest",
            "--public-discussion-ref",
            "eve://epiphany/persona/repo-manifest-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "repo-manifest",
            "--model-ref",
            "repo-manifest-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    let manifest_text = fs::read_to_string(repo.join("epiphany.toml"))
        .with_context(|| format!("failed to read {}", repo.join("epiphany.toml").display()))?;
    ...
    require_text(
        &manifest_text,
        "schema_version = \"epiphany.repo_body_manifest.v0\"",
    )?;
    require_text(
        &manifest_text,
        "safe_action_family = \"repo.body_manifest\"",
    )?;
    require_text(&manifest_text, "[body]")?;
    require_text(&manifest_text, "domain = \"repo:repo-manifest-family\"")?;
    require_text(&manifest_text, "[verses]")?;
    require_text(
        &manifest_text,
        "private = \"epiphany.repo.repo-manifest-family.private\"",
    )?;
    require_text(&manifest_text, "local = \"gamecult-local\"")?;
    require_text(&manifest_text, "public = \"epiphany-global\"")?;
    require_text(&manifest_text, "[eve]")?;
    require_text(
        &manifest_text,
        "surface = \"eve://epiphany/repo/repo-manifest-family\"",
    )?;
    require_text(&manifest_text, "private_state_exposed = false")?;
    require_text(&manifest_text, "arbitrary_shell_authority = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_work_manifest_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "manifestHasBodyDomain": true,
        "manifestHasVerseIds": true,
        "manifestHasEveSurface": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!("expected manifest text to contain {needle:?}"))
    }
}
