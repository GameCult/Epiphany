...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-pr-request-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo PR Request Family Smoke\n\nThis repository proves branch-local PR request cargo without GitHub PR authority.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(["commit", "-m", "Seed repo PR request smoke body"], &repo)?;
    git(["switch", "-c", "epiphany/repo-pr-request-family"], &repo)?;

    let item = "repo-pr-request-family";
    let target_path = ".epiphany/pr-requests/repo-pr-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Bifrost and GitHub to prepare a review PR packet without granting push, PR, or merge authority.",
            "--candidate-action-ref",
            "candidate-action://repo-pr-request-family/pr-packet",
            "--candidate-action-ref",
            "candidate-action://repo-pr-request-family/github-review",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-pr-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "repo-pr-request",
            "--model-ref",
            "repo-pr-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "sync_request_ref = ")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "[pr_packet]")?;
    require_text(&request_text, "base_ref = \"origin/main\"")?;
    require_text(&request_text, "requires_branch_name = true")?;
    require_text(&request_text, "requires_title = true")?;
    require_text(&request_text, "requires_body = true")?;
    require_text(&request_text, "requires_changed_path_list = true")?;
    require_text(&request_text, "requires_public_proof_ref = true")?;
    require_text(&request_text, "requires_maintainer_review_ref = true")?;
    require_text(&request_text, "requires_credit_ref = true")?;
    require_text(
        &request_text,
        "requires_private_state_redaction_check = true",
    )?;
    require_text(&request_text, "github_pr_authorized = false")?;
    require_text(&request_text, "branch_push_authorized = false")?;
    ...
    let summary = json!({
        "schemaVersion": "epiphany.repo_pr_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingPrPublicationReview": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!("expected PR request text to contain {needle:?}"))
    }
}
