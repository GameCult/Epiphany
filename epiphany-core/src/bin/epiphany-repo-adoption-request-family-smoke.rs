...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-adoption-request-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Adoption Request Family Smoke\n\nThis repository proves branch-local Mind adoption requests.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo adoption request smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-adoption-request-family"],
        &repo,
    )?;

    let item = "repo-adoption-request-family";
    let target_path = ".epiphany/adoption-requests/repo-adoption-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Mind to adopt or refuse a reviewable Objective Draft before Self schedules branch-local work.",
            "--candidate-action-ref",
            "candidate-action://repo-adoption-request-family/mind-review",
            "--candidate-action-ref",
            "candidate-action://repo-adoption-request-family/refuse-or-adopt",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-adoption-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-adoption-request",
            "--model-ref",
            "repo-adoption-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "status = \"awaiting-mind-review\"")?;
    require_text(
        &request_text,
        "requested_decision = \"adopt-or-refuse-objective-draft\"",
    )?;
    require_text(&request_text, "mind_review_required = true")?;
    require_text(&request_text, "mind_state_commit_required = true")?;
    require_text(&request_text, "self_scheduling_after_mind_only = true")?;
    require_text(&request_text, "[decision_contract]")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "does_not_modify_state = true")?;
    require_text(&request_text, "objective_draft_required = true")?;
    require_text(&request_text, "consensus_brief_required = true")?;
    require_text(&request_text, "objective_adoption_authorized = false")?;
    ...
    let summary = json!({
        "schemaVersion": "epiphany.repo_work_adoption_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingMindReview": true,
        "doesNotModifyState": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected adoption request text to contain {needle:?}"
        ))
    }
}
