...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Objective Draft Family Smoke\n\nThis repository proves branch-local Objective Draft cargo.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo objective draft smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-objective-draft-family"],
        &repo,
    )?;

    let item = "repo-objective-draft-family";
    let target_path = ".epiphany/objective-drafts/repo-objective-draft-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Shape public consensus into a reviewable Objective Draft before Self schedules any branch-local work.",
            "--candidate-action-ref",
            "candidate-action://repo-objective-draft-family/adopt-objective",
            "--candidate-action-ref",
            "candidate-action://repo-objective-draft-family/write-task-card",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-objective-draft-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-objective-draft",
            "--model-ref",
            "repo-objective-draft-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    let draft_text = fs::read_to_string(repo.join(target_path))
    ...
    require_text(&draft_text, "safe_action_family = \"repo.objective_draft\"")?;
    require_text(&draft_text, "[draft]")?;
    require_text(&draft_text, "status = \"review-required\"")?;
    require_text(&draft_text, "adoption_gate = \"Mind\"")?;
    require_text(&draft_text, "scheduler_gate = \"Self\"")?;
    require_text(&draft_text, "publication_gate = \"Bifrost\"")?;
    require_text(&draft_text, "objective_adopted = false")?;
    require_text(&draft_text, "[objective]")?;
    require_text(
        &draft_text,
        "preferred_next_safe_family = \"repo.task_card\"",
    )?;
    require_text(&draft_text, "[acceptance]")?;
    require_text(&draft_text, "Mind explicitly accepts or rejects")?;
    require_text(&draft_text, "Self schedules only after Mind adoption")?;
    require_text(
        &draft_text,
        "Hands acts only through a later receipt-backed plan",
    )?;
    require_text(&draft_text, "[inputs]")?;
    require_text(&draft_text, "consensus_brief_required = true")?;
    require_text(&draft_text, "objective_adoption_authorized = false")?;
    require_text(&draft_text, "self_scheduling_authorized = false")?;
    require_text(&draft_text, "hands_action_authorized = false")?;
    require_text(&draft_text, "cross_body_mutation_authorized = false")?;
    require_text(&draft_text, "mind_adoption_required = true")?;
    require_text(&draft_text, "bifrost_publication_required = true")?;
    require_text(&draft_text, "private_state_exposed = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_work_objective_draft_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "objectiveAdopted": false,
        "mindAdoptionRequired": true,
        "selfSchedulingAuthorized": false,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected Objective Draft text to contain {needle:?}"
        ))
    }
}
