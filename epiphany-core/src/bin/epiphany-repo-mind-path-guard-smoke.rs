...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-mind-path-guard-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Mind Path Guard Smoke\n\nThis repository proves Mind refuses mismatched requested paths before Hands authority.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo mind path guard smoke body"],
        &repo,
    )?;
    git(["switch", "-c", "epiphany/repo-mind-path-guard"], &repo)?;

    let item = "repo-mind-path-guard";
    let planned_path = "notes/epiphany-work/repo-mind-path-guard.md";
    let tampered_requested_path = "notes/epiphany-work/alien-path.md";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Mind to reject a tampered requested path before Hands receives authority.",
            "--candidate-action-ref",
            "candidate-action://repo-mind-path-guard/tampered-path",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-mind-path-guard",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "planning-note",
            "--model-ref",
            "repo-mind-path-guard-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    tampered_plan["derivation"]["actionItemReceipt"]["requestedPaths"] =
        json!([tampered_requested_path]);
    ...
    if !adopt_failure.contains("do not match plan changed paths") {
        return Err(anyhow!(
            "adopt failure did not name requested-path mismatch: {adopt_failure}"
        ));
    }
    ...
    let refusal_path = repo
        .join(".epiphany")
        .join("work")
        .join("work-mind-adopt-repo-mind-path-guard.json");
    ...
    require_eq(&refusal, &["status"], "refused-requested-path-mismatch")?;
    require_eq(
        &refusal,
    ...
    require_eq(
        &refusal,
        &["interpretation", "inputSummary", "requestedPaths", "0"],
        tampered_requested_path,
    )?;
    require_eq(
        &refusal,
    ...
    require_text_field(
        ...
        "do not match plan changed paths",
    )?;
    require_bool(&refusal, &["gates", "safeFamilyRecognized"], true)?;
    require_bool(&refusal, &["gates", "requestedPathsMatchPlan"], false)?;
    ...
    let adoption_receipt_path = repo
        .join(".epiphany")
        .join("work")
        .join("work-adopt-repo-mind-path-guard.json");
    if adoption_receipt_path.exists() {
        return Err(anyhow!(
            "adoption receipt was written despite requested-path mismatch refusal: {}",
            adoption_receipt_path.display()
        ));
    }

    Ok(json!({
        "schemaVersion": "epiphany.repo_work_mind_path_guard_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        "item": item,
        "tamperedRequestedPath": tampered_requested_path,
        "planChangedPath": planned_path,
        "mindDecisionStatus": refusal["status"],
        "actionItemAccepted": false,
        "safeFamilyRecognized": true,
        "requestedPathsMatchPlan": false,
