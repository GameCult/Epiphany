...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-credit-request-family-{stamp}"));
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Credit Request Family Smoke\n\nThis repository proves branch-local Bifrost credit request cargo without credit ledger authority.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo credit request smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-credit-request-family"],
        &repo,
    )?;

    let item = "repo-credit-request-family";
    let target_path = ".epiphany/credit-requests/repo-credit-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Bifrost to prepare credit ledger review for an accepted artifact without granting credit authority.",
            "--candidate-action-ref",
            "candidate-action://repo-credit-request-family/credit-packet",
            "--candidate-action-ref",
            "candidate-action://repo-credit-request-family/bifrost-ledger-review",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-credit-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "repo-credit-request",
            "--model-ref",
            "repo-credit-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "status = \"awaiting-bifrost-credit-review\"")?;
    ...
    require_text(&request_text, "pr_request_ref = ")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "authorship_context_required = true")?;
    ...
    require_text(&request_text, "[credit_packet]")?;
    require_text(&request_text, "requires_author_identity = true")?;
    ...
    require_text(&request_text, "requires_credit_ledger_target = true")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "bifrost_credit_authority_required = true")?;
    require_text(&request_text, "private_state_exposed = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_credit_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingBifrostCreditReview": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected credit request text to contain {needle:?}"
        ))
    }
}
