...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-sync-request-family-{stamp}"));
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Sync Request Family Smoke\n\nThis repository proves branch-local Hands sync-request requests.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(["commit", "-m", "Seed repo sync request smoke body"], &repo)?;
    git(["switch", "-c", "epiphany/repo-sync-request-family"], &repo)?;

    let item = "repo-sync-request-family";
    let target_path = ".epiphany/sync-requests/repo-sync-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Bifrost to prove the published commit is contained by upstream main after maintainer review.",
            "--candidate-action-ref",
            "candidate-action://repo-sync-request-family/upstream-main-proof",
            "--candidate-action-ref",
            "candidate-action://repo-sync-request-family/ancestry-proof",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-sync-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "repo-sync-request",
            "--model-ref",
            "repo-sync-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "publication_receipt_required = true")?;
    require_text(&request_text, "github_publication_required = true")?;
    require_text(&request_text, "maintainer_review_required = true")?;
    require_text(&request_text, "credit_ledger_required = true")?;
    ...
    require_text(&request_text, "[sync_proof]")?;
    require_text(&request_text, "target_ref = \"origin/main\"")?;
    require_text(&request_text, "requires_fetch_before_check = true")?;
    require_text(&request_text, "requires_merge_base_ancestor_check = true")?;
    require_text(&request_text, "requires_clean_public_proof_readback = true")?;
    require_text(&request_text, "records_upstream_main_sha = true")?;
    ...
    let summary = json!({
        "schemaVersion": "epiphany.repo_sync_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingUpstreamMainProof": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!("expected sync request text to contain {needle:?}"))
    }
}
