...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-scheduling-request-family-{stamp}"));
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Scheduling Request Family Smoke\n\nThis repository proves branch-local Self scheduling requests.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo scheduling request smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-scheduling-request-family"],
        &repo,
    )?;

    let item = "repo-scheduling-request-family";
    let target_path = ".epiphany/scheduling-requests/repo-scheduling-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            "repo-scheduling-request-family-smoke",
            "--topic",
            "repo-scheduling-request-family",
        ],
        &root,
    )?;
    cargo_json(
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask Self to prepare queue scheduling only after Mind adopts the Objective Draft.",
            "--candidate-action-ref",
            "candidate-action://repo-scheduling-request-family/queue-after-adoption",
            "--candidate-action-ref",
            "candidate-action://repo-scheduling-request-family/bounded-self-pulse",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-scheduling-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-scheduling-request",
            "--model-ref",
            "repo-scheduling-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "status = \"awaiting-mind-adoption\"")?;
    require_text(&request_text, "requested_scheduler = \"Self\"")?;
    require_text(&request_text, "mind_adoption_receipt_required = true")?;
    require_text(&request_text, "self_may_schedule_after_mind_only = true")?;
    require_text(&request_text, "queue_run_allowed_after_adoption = true")?;
    require_text(&request_text, "[queue]")?;
    require_text(&request_text, "target_gate = \"repo-work-queue\"")?;
    require_text(
        &request_text,
        "preferred_next_safe_family = \"repo.task_card\"",
    )?;
    require_text(&request_text, "max_items_per_pulse = 1")?;
    require_text(&request_text, "requires_epiphany_branch = true")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "self_scheduling_authorized = false")?;
    require_text(&request_text, "queue_mutation_authorized = false")?;
    ...
    let summary = json!({
        "schemaVersion": "epiphany.repo_work_scheduling_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected scheduling request text to contain {needle:?}"
        ))
    }
}
