...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-tool-capabilities-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Tool Capabilities Family Smoke\n\nThis repository proves branch-local Epiphany tool capability manifests.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo tool capabilities smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-tool-capabilities-family"],
        &repo,
    )?;

    let item = "repo-tool-capabilities-family";
    let target_path = ".epiphany/repo-tool-capabilities.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Publish a branch-local repo tool capability manifest with CultMesh intent and receipt contracts for authorized agents.",
            "--candidate-action-ref",
            "candidate-action://repo-tool-capabilities-family/tool-directory",
            "--public-discussion-ref",
            "eve://epiphany/persona/repo-tool-capabilities-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-tool-capabilities",
            "--model-ref",
            "repo-tool-capabilities-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    let manifest_text = fs::read_to_string(repo.join(target_path))
    ...
    require_text(
        &manifest_text,
        "schema_version = \"epiphany.repo_tool_capabilities.v0\"",
    )?;
    require_text(
        &manifest_text,
        "safe_action_family = \"repo.tool_capabilities\"",
    )?;
    require_text(&manifest_text, "[tool_directory]")?;
    require_text(&manifest_text, "verse = \"gamecult-local\"")?;
    require_text(&manifest_text, "odin_discoverable = true")?;
    require_text(&manifest_text, "available_to_authorized_agents = true")?;
    require_text(&manifest_text, "[contracts]")?;
    require_text(
        &manifest_text,
    ...
    require_text(&manifest_text, "host_daemon_owns_execution = true")?;
    require_text(&manifest_text, "idunn_owns_lifecycle = true")?;
    require_text(&manifest_text, "[expected_capabilities]")?;
    require_text(&manifest_text, "\"repo-work-overview\"")?;
    require_text(&manifest_text, "\"repo-work-queue-run\"")?;
    require_text(&manifest_text, "\"repo-work-public-proof\"")?;
    require_text(&manifest_text, "\"bifrost-public-proof\"")?;
    require_text(&manifest_text, "arbitrary_shell_authority = false")?;
    require_text(&manifest_text, "deployment_authority = false")?;
    require_text(&manifest_text, "service_start_stop_authority = false")?;
    require_text(&manifest_text, "private_verse_rummaging = false")?;
    require_text(
        &manifest_text,
        "tool_invocation_requires_host_receipt = true",
    )?;
    require_text(&manifest_text, "private_state_exposed = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_work_tool_capabilities_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "manifestHasToolDirectory": true,
        "manifestHasContracts": true,
        "manifestHasExpectedCapabilities": true,
        "hostDaemonOwnsExecution": true,
        "idunnOwnsLifecycle": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected capability manifest text to contain {needle:?}"
        ))
    }
}
