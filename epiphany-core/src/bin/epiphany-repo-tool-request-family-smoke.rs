...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-tool-request-family-{stamp}"));
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Tool Request Family Smoke\n\nThis repository proves branch-local Epiphany daemon tool request cargo.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(["commit", "-m", "Seed repo tool request smoke body"], &repo)?;
    git(["switch", "-c", "epiphany/repo-tool-request-family"], &repo)?;

    let item = "repo-tool-request-family";
    let target_path = ".epiphany/tool-requests/repo-tool-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Request a daemon-hosted tool through CultMesh typed intent and receipt contracts without granting direct execution authority.",
            "--candidate-action-ref",
            "candidate-action://repo-tool-request-family/daemon-intent",
            "--public-discussion-ref",
            "eve://epiphany/persona/repo-tool-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "repo-tool-request",
            "--model-ref",
            "repo-tool-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "operation = \"submitTypedToolIntent\"")?;
    require_text(&request_text, "[cultmesh]")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "discoverable = true")?;
    require_text(&request_text, "preserves_provider_ownership = true")?;
    require_text(&request_text, "private_verse_passthrough = false")?;
    require_text(&request_text, "[authority]")?;
    require_text(&request_text, "direct_tool_execution = false")?;
    require_text(&request_text, "arbitrary_shell_authority = false")?;
    require_text(&request_text, "hands_action_authority = false")?;
    require_text(&request_text, "state_commit_authority = false")?;
    require_text(&request_text, "publication_authority = false")?;
    require_text(&request_text, "service_lifecycle_authority = false")?;
    require_text(&request_text, "cross_body_mutation_authority = false")?;
    require_text(&request_text, "private_verse_rummaging = false")?;
    require_text(&request_text, "private_state_exposed = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_work_tool_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "hasCultMeshContracts": true,
        "hostDaemonOwnsExecution": true,
        "odinPreservesProviderOwnership": true,
        "directToolExecution": false,
        "arbitraryShellAuthority": false,
        "handsActionAuthority": false,
        "stateCommitAuthority": false,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!("expected tool request text to contain {needle:?}"))
    }
}
