...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-secret-policy-request-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Secret Policy Request Family Smoke\n\nThis repository proves branch-local secret policy request cargo without secret access.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    fs::write(
        repo.join(".env.example"),
        "API_TOKEN=example-only-no-secret-value\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join(".env.example").display()))?;
    git(["add", "README.md", ".env.example"], &repo)?;
    git(
        ["commit", "-m", "Seed repo secret policy smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-secret-policy-request-family"],
        &repo,
    )?;

    let item = "repo-secret-policy-request-family";
    let target_path =
    ...
    cargo_json(
        ...
        &[
            ...
            "repo-secret-policy-request-family-smoke",
            "--topic",
            "repo-secret-policy-request-family",
        ],
        &root,
    )?;
    cargo_json(
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask for a reviewed repo secret and write-permission policy without exposing secret values.",
            "--candidate-action-ref",
            "candidate-action://repo-secret-policy-request-family/security-review",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-secret-policy-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            "--model-ref",
            "repo-secret-policy-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    let env_example_text = fs::read_to_string(repo.join(".env.example"))
        .with_context(|| format!("failed to read {}", repo.join(".env.example").display()))?;
    ...
    require_text(&request_text, "requires_write_permission_scope = true")?;
    require_text(
        &request_text,
        "requires_public_private_export_boundary = true",
    )?;
    require_text(&request_text, "source_grounding_required = true")?;
    require_text(&request_text, "bifrost_publication_review_required = true")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "requires_credential_owner = true")?;
    require_text(&request_text, "requires_write_scope_matrix = true")?;
    require_text(
        &request_text,
        "requires_public_export_redaction_rules = true",
    )?;
    require_text(&request_text, "requires_deployment_authority_owner = true")?;
    require_text(&request_text, "requires_incident_rollback_plan = true")?;
    require_text(&request_text, "direct_secret_access_authority = false")?;
    require_text(&request_text, "secret_value_materialization = false")?;
    require_text(&request_text, "write_permission_authority = false")?;
    ...
    require_text(&env_example_text, "example-only-no-secret-value")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_secret_policy_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingSecurityReview": true,
        "secretAccessAuthorized": false,
        "writePermissionAuthorized": false,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected secret policy request text to contain {needle:?}"
        ))
    }
}
