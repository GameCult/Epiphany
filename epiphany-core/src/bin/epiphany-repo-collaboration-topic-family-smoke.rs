...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-collaboration-topic-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Collaboration Topic Family Smoke\n\nThis repository proves branch-local Epiphany collaboration topics.\n",
    )
    .with_context(|| format!("failed to seed {}", repo.join("README.md").display()))?;
    git(["add", "README.md"], &repo)?;
    git(
        ["commit", "-m", "Seed repo collaboration topic smoke body"],
        &repo,
    )?;
    git(
        ["switch", "-c", "epiphany/repo-collaboration-topic-family"],
        &repo,
    )?;

    let item = "repo-collaboration-topic-family";
    let target_path = ".epiphany/collaboration-topics/repo-collaboration-topic-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            "repo-collaboration-topic-family-smoke",
            "--topic",
            "repo-collaboration-topic-family",
        ],
        &root,
    )?;
    cargo_json(
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Open a public Eve collaboration topic that routes Persona and human feedback to Imagination consensus before any repo action.",
            "--candidate-action-ref",
            "candidate-action://repo-collaboration-topic-family/eve-topic",
            "--candidate-action-ref",
            "candidate-action://repo-collaboration-topic-family/imagination-consensus",
            "--public-discussion-ref",
            "eve://epiphany/persona/repo-collaboration-topic-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-collaboration-topic",
            "--model-ref",
            "repo-collaboration-topic-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    let topic_text = fs::read_to_string(repo.join(target_path))
    ...
    require_text(&topic_text, "[imagination]")?;
    require_text(
        &topic_text,
        "consensus_route = \"imagination://repo/repo-collaboration-topic-family/consensus-discovery\"",
    )?;
    require_text(&topic_text, "consensus_required_before_action = true")?;
    require_text(
        &topic_text,
        "candidate_actions_are_non_authoritative = true",
    )?;
    require_text(&topic_text, "mind_adoption_required = true")?;
    require_text(&topic_text, "bifrost_publication_required = true")?;
    require_text(&topic_text, "adoption_authorized = false")?;
    require_text(&topic_text, "hands_action_authorized = false")?;
    require_text(&topic_text, "cross_body_mutation_authorized = false")?;
    require_text(&topic_text, "private_verse_rummaging = false")?;
    require_text(&topic_text, "private_state_exposed = false")?;

    let summary = json!({
        "schemaVersion": "epiphany.repo_work_collaboration_topic_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "topicHasPublicRoom": true,
        "topicHasEveSurface": true,
        "topicHasImaginationRoute": true,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected collaboration topic text to contain {needle:?}"
        ))
    }
}
