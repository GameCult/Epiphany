...
fn run_smoke(args: Args) -> Result<Value> {
    ...
    let smoke_dir = smoke_root.join(format!("repo-maintainer-review-request-family-{stamp}"));
    if smoke_dir.exists() {
    ...
    fs::write(
        repo.join("README.md"),
        "# Repo Maintainer Review Request Family Smoke\n\nThis repository proves branch-local Hands maintainer-review-request requests.\n",
    )
    ...
    git(
        [
            "commit",
            "-m",
            "Seed repo maintainer review request smoke body",
        ],
        &repo,
    )?;
    git(
        [
            "switch",
            "-c",
            "epiphany/repo-maintainer-review-request-family",
        ],
        &repo,
    )?;

    let item = "repo-maintainer-review-request-family";
    let target_path = ".epiphany/review-requests/repo-maintainer-review-request-family.toml";
    ...
    cargo_json(
        ...
        &[
            ...
            "repo-maintainer-review-request-family-smoke",
            "--topic",
            "repo-maintainer-review-request-family",
        ],
        &root,
    )?;
    cargo_json(
    ...
    cargo_json(
        ...
        &[
            ...
            item,
            "--summary",
            "Ask a maintainer to review the redacted proof and branch diff without granting merge authority.",
            "--candidate-action-ref",
            "candidate-action://repo-maintainer-review-request-family/human-review",
            "--candidate-action-ref",
            "candidate-action://repo-maintainer-review-request-family/maintainer-verdict",
            "--public-discussion-ref",
            "epiphany-global/persona-collaboration/repo-maintainer-review-request-family",
        ],
        &root,
    )?;
    let plan = cargo_json(
        ...
        &[
            ...
            item,
            "--action-family",
            "repo-maintainer-review-request",
            "--model-ref",
            "repo-maintainer-review-request-family-smoke-imagination-v1",
            "--model-authored",
        ],
        &root,
    )?;
    ...
    require_text(&request_text, "status = \"awaiting-maintainer-review\"")?;
    require_text(&request_text, "requested_owner = \"Maintainer\"")?;
    require_text(
        &request_text,
    ...
    require_text(&request_text, "bifrost_publication_request_required = true")?;
    ...
    require_text(&request_text, "[review_packet]")?;
    ...
    let summary = json!({
        "schemaVersion": "epiphany.repo_maintainer_review_request_family_smoke.v0",
        "status": "ok",
        "smokeDir": smoke_dir,
        "repo": repo,
        ...
        "awaitingMaintainerReview": true,
        "maintainerApprovalAuthorized": false,
...
fn require_text(haystack: &str, needle: &str) -> Result<()> {
    if haystack.contains(needle) {
        ...
        Err(anyhow!(
            "expected maintainer review request text to contain {needle:?}"
        ))
    }
}
