...
pub fn protocol_scene(scene: CoreEpiphanyScene) -> ThreadEpiphanyScene {
    ThreadEpiphanyScene {
        ...
        active_subgoal: scene.active_subgoal.map(protocol_scene_subgoal),
        subgoals: scene
            .subgoals
            .into_iter()
            .map(protocol_scene_subgoal)
            .collect(),
        invariant_status_counts: scene
            .invariant_status_counts
            .into_iter()
            .map(protocol_scene_status_count)
            .collect(),
        graph: protocol_scene_graph(scene.graph),
        retrieval: scene.retrieval.map(protocol_scene_retrieval),
        investigation_checkpoint: scene
            .investigation_checkpoint
            .map(protocol_scene_investigation_checkpoint),
        observations: protocol_scene_records(scene.observations),
        evidence: protocol_scene_records(scene.evidence),
        churn: scene.churn.map(protocol_scene_churn),
...
pub fn protocol_scene_action(action: CoreEpiphanySceneAction) -> ThreadEpiphanySceneAction {
    match action {
...
fn protocol_scene_subgoal(subgoal: CoreEpiphanySceneSubgoal) -> ThreadEpiphanySceneSubgoal {
...
fn protocol_scene_status_count(
...
fn protocol_scene_graph(graph: CoreEpiphanySceneGraph) -> ThreadEpiphanySceneGraph {
...
fn protocol_scene_retrieval(retrieval: CoreEpiphanySceneRetrieval) -> ThreadEpiphanySceneRetrieval {
...
fn protocol_scene_investigation_checkpoint(
...
fn protocol_scene_records(records: CoreEpiphanySceneRecords) -> ThreadEpiphanySceneRecords {
    ThreadEpiphanySceneRecords {
        total_count: records.total_count,
        latest: records
            .latest
            .into_iter()
            .map(protocol_scene_record)
            .collect(),
    }
}

fn protocol_scene_record(record: CoreEpiphanySceneRecord) -> ThreadEpiphanySceneRecord {
...
fn protocol_scene_churn(churn: CoreEpiphanySceneChurn) -> ThreadEpiphanySceneChurn {
